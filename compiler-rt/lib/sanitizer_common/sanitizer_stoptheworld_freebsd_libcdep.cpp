...
//
// This is a FreeBSD variation of Linux stoptheworld implementation
...
class SuspendedThreadsListFreeBSD final : public SuspendedThreadsList {
 public:
  SuspendedThreadsListFreeBSD() { thread_ids_.reserve(1024); }
...
class ThreadSuspender {
 ...
 private:
  SuspendedThreadsListFreeBSD suspended_threads_list_;
  pid_t pid_;
};
...
bool ThreadSuspender::SuspendAllThreads() {
  ...
  int val;
  while ((val = internal_ptrace(op, pid_, (void *)&pl, (void *)sizeof(pl))) != -1 &&
         pl.pl_lwpid != 0) {
...
void StopTheWorld(StopTheWorldCallback callback, void *argument) {
  ...
  // tracer_pid = rfork_thread(RFMEM, tracer_stack.Bottom(), TracerThread, &tracer_thread_argument);
#endif
  tracer_pid = fork();
  if (tracer_pid == -1) {
...
void SuspendedThreadsListFreeBSD::Append(tid_t tid) {
...
PtraceRegistersStatus SuspendedThreadsListFreeBSD::GetRegistersAndSP(
  ...
  struct reg regs;
  int res = internal_ptrace(PT_GETREGS, tid, &regs, 0);
  if (res < 0) {
    VReport(1,
            "Could not get registers from process %d thread %d (errno %d).\n",
            ppid, tid, -res);
    return -res == ESRCH ? REGISTERS_UNAVAILABLE_FATAL
