...
ELFDumper<ELFT>::ELFDumper(const object::ELFObjectFile<ELFT> &O,
    ...
    : ObjDumper(Writer, O.getFileName()), ObjF(O), Obj(O.getELFFile()),
      ...
      DynRelaRegion(O, *this), DynRellebRegion(O, *this),
...
template <typename ELFT> void ELFDumper<ELFT>::parseDynamicTable() {
  ...
  for (const Elf_Dyn &Dyn : dynamic_table()) {
    ...
    case ELF::DT_RELAENT:
      ...
      break;
    case ELF::DT_RELLEB:
      DynRellebRegion.Addr = toMappedAddr(Dyn.getTag(), Dyn.getPtr());
      break;
    case ELF::DT_SONAME:
...
static void printRelocHeaderFields(formatted_raw_ostream &OS, unsigned SType,
  ...
  bool IsRela = is_contained(
      {ELF::SHT_RELA, ELF::SHT_RELLEB, ELF::SHT_ANDROID_RELA}, SType);
  bool IsRelr =
...
static bool isRelocationSec(const typename ELFT::Shdr &Sec,
  ...
  return Sec.sh_type == ELF::SHT_REL || Sec.sh_type == ELF::SHT_RELA ||
         Sec.sh_type == ELF::SHT_RELR || Sec.sh_type == ELF::SHT_RELLEB ||
...
template <class ELFT> void GNUELFDumper<ELFT>::printRelocations() {
  auto GetEntriesNum = [&](const Elf_Shdr &Sec) -> Expected<size_t> {
    ...
    if (Sec.sh_type == ELF::SHT_RELLEB) {
      if (Expected<std::vector<Elf_Rela>> RelasOrErr = this->Obj.rellebs(Sec))
        return RelasOrErr->size();
      else
        return RelasOrErr.takeError();
    }
...
template <class ELFT> void ELFDumper<ELFT>::printDynamicRelocationsHelper() {
  ...
  if (this->DynRellebRegion.Size > 0) {
    printDynamicRelocHeader(ELF::SHT_RELLEB, "RELLEB", this->DynRellebRegion);
    // While the size is unknown, a valid RELLEB has at least one byte. We can
    // check whether Addr is in bounds, and then decode RELLEB until the file
    // end.
    this->DynRellebRegion.Size = 1;
    if (!this->DynRellebRegion.template getAsArrayRef<uint8_t>().empty()) {
      const uint64_t Offset =
          DynRellebRegion.Addr -
          (const uint8_t *)ObjF.getMemoryBufferRef().getBufferStart();
      const uint64_t ObjSize = ObjF.getMemoryBufferRef().getBufferSize();
      auto RelasOrErr = Obj.decodeRelleb(
          ArrayRef<uint8_t>(DynRellebRegion.Addr, ObjSize - Offset));
      if (!RelasOrErr) {
...
void ELFDumper<ELFT>::forEachRelocationDo(
  ...
  case ELF::SHT_RELLEB: {
    if (Expected<std::vector<Elf_Rela>> RelasOrErr = Obj.rellebs(Sec)) {
