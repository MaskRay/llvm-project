...
void PrinterContext<ELFT>::printEHFrameHdr(const Elf_Phdr *EHFramePHdr) const {
  ...
  // Version 2 uses 4-byte initial_location + 8-byte value (12-byte entries).
  // Version 1 uses TableEntrySize for both fields.
  unsigned EntrySize = Version == 2 ? 12 : 2 * TableEntrySize;
  unsigned InitialLocSize = Version == 2 ? 4 : TableEntrySize;

  unsigned NumEntries = 0;
  uint64_t PrevPC = 0;
  while (Offset + EntrySize <= EHFramePHdr->p_memsz && NumEntries < FDECount) {
    DictScope D(W, std::string("entry ") + std::to_string(NumEntries));

    auto InitialPC = DE.getSigned(&Offset, InitialLocSize) + EHFrameHdrAddress;
