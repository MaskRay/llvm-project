...
#endif
    // Round up to MinAlign to keep CurPtr aligned after the bump. For constant
    // sizes this folds away, unlike the CurPtr-dependent rounding below.
    SizeToAllocate = alignToPowerOf2(SizeToAllocate, MinAlign);

    // CurPtr is already MinAlign-aligned, so a request aligned no more than
    // that starts right at CurPtr. The branch folds away for a constant
    // Alignment.
    uintptr_t AlignedPtr = Alignment.value() <= MinAlign
                               ? reinterpret_cast<uintptr_t>(CurPtr)
