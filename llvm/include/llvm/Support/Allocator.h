...
public:
  ...
  LLVM_ATTRIBUTE_RETURNS_NONNULL void *Allocate(size_t Size, Align Alignment) {
    ...
    // Bump down from CurPtr. CurPtr stays MinAlign-aligned, so only a stricter
    // request realigns.
    uintptr_t NewPtr = uintptr_t(CurPtr) - SizeToAllocate;
    if (Alignment.value() > MinAlign)
      NewPtr &= -Alignment.value();

    // An empty allocator (CurPtr != nullptr) takes the slow path.
...
private:
  /// [End, CurPtr) is the free region in the current slab.
  char *CurPtr = nullptr;
  char *End = nullptr;
