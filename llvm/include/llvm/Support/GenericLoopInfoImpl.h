...
void LoopInfoBase<BlockT, LoopT>::analyze(const DomTreeBase<BlockT> &DomTree) {
  ...
  // tallying each loop's blocks in BlockLen. A loop's header is emitted last
  // among its blocks (it dominates them all), so on reaching it BlockLen is the
  // loop's final inclusive count: fold it into the parent, or register a
  // top-level loop, in postorder of headers.
  ...
  // Walk the recorded postorder in reverse (RPO). The first block of a loop
  // reached is its header, as it dominates the loop's blocks; carve its slice
  // from the parent's cursor there (the parent's slice already exists, its
  // header even earlier) and register it as a subloop. Each block lands once,
  // at its innermost loop's cursor.
