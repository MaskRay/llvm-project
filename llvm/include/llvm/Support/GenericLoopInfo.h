...
public:
  ...
  bool contains(const BlockT *BB) const {
    assert(!isInvalid() && "Loop not in a valid state!");
    return contains(LI->getLoopForTolerant(BB));
  }

  /// Return true if \p BB appears in this loop's block list.
  ///
  /// Unlike contains(const BlockT *), which consults LoopInfo's central
  /// block-to-loop map, this scans this loop's own block list. It therefore
  /// stays valid while the loop nest is being restructured, when the map can be
  /// transiently out of sync with the block lists (e.g. a block has been added
  /// to this loop but not yet mapped to it, or a subloop's blocks are listed
  /// here before the subloop is attached). This is O(number of blocks); prefer
  /// contains() outside of such transient states.
  bool containsBlockRaw(const BlockT *BB) const {
    assert(!isInvalid() && "Loop not in a valid state!");
...
public:
  ...
  LoopInfoBase(LoopInfoBase &&Arg)
    ...
    // The loops' owning-LoopInfo back-pointers now refer to the moved-from
    // object; repoint them at this one.
    resetLoopInfoOwners();
...
private:
  ...
  // after a move, since the loops themselves (bump-allocated) do not move but
  // their owner's address does. Walks the forest with a single reused worklist
  // rather than getLoopsInPreorder(), which materializes a throwaway vector.
  ...
  // Look up BB's innermost loop in the block-to-loop map. Does not check that
  // BB belongs to this function; callers must ensure that (see getLoopFor and
  // getLoopForTolerant).
  ...
  // Like getLoopFor, but tolerates a block from another function (or one not
  // yet inserted) by returning nullptr rather than asserting. Used by
  // LoopBase::contains(BlockT *), which may be queried with a block outside
  // this LoopInfo's function (e.g. a use of a global in another function).
  LoopT *getLoopForTolerant(const BlockT *BB) const {
    // A foreign block's number would index this function's map and could
    // return an unrelated loop, so filter by function first.
    if constexpr (GraphHasNodeNumbers<const BlockT *>)
      if (BB->getParent() != ParentPtr)
        return nullptr;
