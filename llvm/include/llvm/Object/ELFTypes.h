...
template <bool Is64> struct Elf_Relleb_Impl;
...
public:
  ...
  using Relleb = Elf_Relleb_Impl<Is64>;
...
#define LLVM_ELF_IMPORT_TYPES_ELFT(ELFT)                                       \
  ...
  using Elf_Relleb = typename ELFT::Relleb;                                    \
...
struct Elf_Rel_Impl<ELFType<TargetEndianness, false>, false> {
  ...
  static const bool IsRelleb = false;
...
template <bool Is64> struct Elf_Relleb_Impl {
  using uint = std::conditional_t<Is64, uint64_t, uint32_t>;
  static const bool IsRela = true;
  static const bool IsRelleb = true;
  uint r_offset;
  uint32_t r_symidx;
  uint32_t r_type;
