...
public:
  ...
  /// other argument to \p Rest in order. On a missing value, or a value
  /// \p Apply rejects, writes a line to \p Errs, which the caller may prefix
  /// with the program name, and returns false.
  ...
  /// option like applyArgs reports an error.
