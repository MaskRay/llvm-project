...
public:
  ...
  /// Fields that few options set, kept out of Info. Row 0 of the table is all
  /// zero and serves every option without an InfoExtra of its own.
  struct InfoExtra {
...
private:
  ...
  StringTable::Offset getHelpTextOffset(const Info &I,
    ...
    if (unsigned Offset = getExtra(I).HelpTextVariantsOffset)
      for (const HelpTextVariant *V = &HelpTextVariantsTable[Offset];
           V->Visibility; ++V)
  ...
  ArrayRef<unsigned> getSubCommandIDs(const Info &I) const {
    ...
    return Offset ? SubCommandIDsTable.slice(Offset + 1,
                                             SubCommandIDsTable[Offset])
                  : ArrayRef<unsigned>();
  }
