...
// Forward cv-qualified types to the unqualified trait, so a type only has to
// specialize the unqualified form.  hash_combine_range deduces its element type
// as `const T` from an ArrayRef's iterators, which would otherwise miss a
// specialization written for `T`.
