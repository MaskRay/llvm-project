...
inline hash_code combine_bytes(const char *data, size_t len) {
  return ::llvm::xxh3_64bits(reinterpret_cast<const uint8_t *>(data), len) ^
         get_execution_seed();
}
...
template <typename... Ts> hash_code hash_combine(const Ts &...args) {
  constexpr size_t Total =
      ::llvm::hashing::detail::total_hashable_size<Ts...>();
  ...
  size_t off = 0;
  (::llvm::hashing::detail::store_hashable_data(buf.data(), off, args), ...);
  return ::llvm::hashing::detail::combine_bytes(buf.data(), Total);
}
