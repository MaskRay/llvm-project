...
void GenericCycleInfo<ContextT>::moveTopLevelCycleToNewParent(CycleT *NewParent,
  ...
  *Pos = std::move(CurrentContainer.back());
  ...
  // Rebuild the shared block layout so NewParent's slice now covers Child's
  // blocks. Reparenting leaves the innermost-cycle map unchanged, so the
  // current layout already lists every in-cycle block once and seeds the
  // rebuild (preserving relative order).
  SmallVector<BlockT *, 8> Order(BlockLayout.begin(), BlockLayout.end());
  layoutBlocks(Order);
...
void GenericCycleInfoCompute<ContextT>::run(FunctionT *F) {
  ...
  // Record each block's innermost containing cycle and tally that cycle's own
  // block count on the fly (fusing what would otherwise be a separate counting
  // pass in layoutBlocks). The per-cycle block ranges are laid out from this
  // map once the cycle tree is wired up (see below).
  for (BlockT *Block : Preorder)
    if (BlockT *H = innermostHeader(Block)) {
      CycleT *C = HeaderToCycle[num(H)];
      Info.BlockMap[num(Block)] = C;
      ++C->OwnBlocks;
    }
  ...
  // Lay out the shared block storage now that the tree is wired: each cycle's
  // blocks (its own and those of nested cycles) form a contiguous slice of
  // BlockLayout, headers first, in preorder.
  Info.layoutBlocks(Preorder);
