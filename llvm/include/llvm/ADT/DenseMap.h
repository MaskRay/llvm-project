...
// grow's callers pass a power of two, leaving the minimum table size to apply.
constexpr unsigned roundUpNumBuckets(unsigned MinNumBuckets) {
  assert((MinNumBuckets == 0 || isPowerOf2_64(MinNumBuckets)) &&
         "bucket count must be zero or a power of two");
  return std::max(64u, MinNumBuckets);
}

/// Hashes the key, at offset 0 in a bucket. Null asks the loops below to inline
/// the pointer hash.
using BucketHasher = unsigned (*)(const void *Key);

// True when Info declares the tag naming itself, which a specialization of
// DenseMapInfo for one pointer type replaces along with the hash.
template <typename Info, typename = void>
inline constexpr bool declaresPointerValueHash = false;
template <typename Info>
inline constexpr bool declaresPointerValueHash<
    Info,
    std::enable_if_t<std::is_same_v<typename Info::PointerValueHash, Info>>> =
    true;

// True when KeyInfoT hashes the key by its pointer value. Only
// DenseMapInfo<KeyT> is asked for the tag: an info inheriting it may hash the
// pointee, and GCC without DR1170 errors on a lookup that reaches a private
// base, as clang::WeakInfo's info has.
...
constexpr bool hashesPointerValue() {
  if constexpr (std::is_pointer_v<KeyT> &&
    ...
    return declaresPointerValueHash<KeyInfoT>;
  else
    return false;
}
...
/// room for all of them; \p Src keeps the copied bytes for its owner to free.
LLVM_ABI void rehashRelocatable(char *Dst, UsedT *DstUsed,
...
/// Return a table of \p NewNumBuckets buckets holding the live entries of the
/// \p OldNumBuckets buckets at \p OldBuckets, which it frees; a null
/// \p OldBuckets just allocates.
LLVM_ABI char *growRelocatable(char *OldBuckets, unsigned OldNumBuckets,
                               unsigned NewNumBuckets, BucketTraits Traits,
                               BucketHasher Hasher);
...
protected:
  ...
  static constexpr llvm::densemap::detail::BucketTraits traits() {
    return {sizeof(BucketT), llvm::densemap::detail::allocAlign<BucketT>()};
  }
...
private:
  ...
  void growShared(unsigned MinNumBuckets) {
    ...
    char *Storage = llvm::densemap::detail::growRelocatable(
        reinterpret_cast<char *>(Buckets), NumBuckets, NewNumBuckets,
        BaseT::traits(), BaseT::hasher());
    Buckets = reinterpret_cast<BucketT *>(Storage);
    NumBuckets = NewNumBuckets;
    Used = llvm::densemap::detail::usedFor(Storage, sizeof(BucketT),
                                           NewNumBuckets);
  }
...
private:
  ...
  void setLarge(char *Storage, unsigned NumBuckets) {
    Small = false;
    storage.Large = {
        reinterpret_cast<BucketT *>(Storage),
        llvm::densemap::detail::usedFor(Storage, sizeof(BucketT), NumBuckets),
        NumBuckets};
  }
  ...
  void growShared(unsigned MinNumBuckets) {
    const unsigned NewNumBuckets = roundUpNumBuckets(MinNumBuckets);
    if (!Small) {
      setLarge(llvm::densemap::detail::growRelocatable(
                   reinterpret_cast<char *>(storage.Large.Buckets),
                   storage.Large.NumBuckets, NewNumBuckets, BaseT::traits(),
                   BaseT::hasher()),
               NewNumBuckets);
      return;
    }

    // remove_if asks for the count it already has. Cold, so it shares the loop.
    if (NewNumBuckets <= InlineBuckets) {
      ...
      llvm::densemap::detail::rehashRelocatable(
          storage.Inline.Buckets, getInlineUsed(), InlineBuckets, Old.Buckets,
          Old.Used, InlineBuckets, sizeof(BucketT), BaseT::hasher());
      return;
    }

    // A spill relocates at most InlineBuckets entries, too few to pay the call.
    char *Storage = llvm::densemap::detail::growRelocatable(
        nullptr, 0, NewNumBuckets, BaseT::traits(), BaseT::hasher());
    BucketT *NewB = reinterpret_cast<BucketT *>(Storage);
    UsedT *NewU = llvm::densemap::detail::usedFor(Storage, sizeof(BucketT),
                                                  NewNumBuckets);
    const BucketT *OldB = getInlineBuckets();
    const unsigned Mask = NewNumBuckets - 1;
    llvm::densemap::detail::forEachUsed(
        getInlineUsed(), InlineBuckets, [&](unsigned I) {
          unsigned BucketNo = KeyInfoT::getHashValue(OldB[I].getFirst()) & Mask;
          while (llvm::densemap::detail::used(NewU, BucketNo))
            BucketNo = (BucketNo + 1) & Mask;
          std::memcpy(reinterpret_cast<void *>(NewB + BucketNo), &OldB[I],
                      sizeof(BucketT));
          llvm::densemap::detail::setUsed(NewU, BucketNo);
        });
  ...
  bool allocateBuckets(unsigned Num) {
    ...
    setLarge(static_cast<char *>(allocate_buffer(
                 llvm::densemap::detail::allocBytes<BucketT>(Num),
                 llvm::densemap::detail::allocAlign<BucketT>())),
             Num);
    return true;
  }
