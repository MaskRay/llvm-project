...
/// deletion (Knuth TAOCP 6.4 Algorithm R), power-of-two capacity, a 64-bucket
/// minimum, and a 0.75 maximum load factor. No sentinel key. Occupancy is
/// stored in a packed 1-bit-per-bucket "used" array.
///
/// `SmallDenseMap` adds an inline small-buffer optimization: up to
/// `InlineBuckets` buckets (and their parallel used array) live in an inline
...
// A shared dummy used word used by empty maps (NumBuckets == 0).
inline used_t EmptyUsed = 0;

// Number of used words backing N buckets where N is zero or a power of two.
constexpr size_t usedWords(unsigned N) {
  assert((N == 0 || isPowerOf2_32(N)) &&
...
inline void setUnused(used_t *U, size_t I) {
  U[I >> 5] &= ~(used_t(1) << (I & 31));
}
...
class DenseMap : public DenseMapBase<DenseMap<KeyT, ValueT, KeyInfoT, BucketT>,
  ...
  used_t *Used = &llvm::densemap::detail::EmptyUsed;
...
private:
  ...
  void deallocateBuckets() {
    ...
    Buckets = nullptr;
    Used = &llvm::densemap::detail::EmptyUsed;
    NumBuckets = 0;
  }
...
private:
  ...
  void kill() {
    deallocateBuckets();
    Small = false;
    storage.Large = LargeRep{nullptr, &llvm::densemap::detail::EmptyUsed, 0};
  }
...
private:
  void AdvancePastEmptyBuckets() {
    ...
    if constexpr (shouldReverseIterate<KeyT>()) {
      ...
      // the end iterator is never dereferenced; &*Ptr works for a
      // std::reverse_iterator<pointer>.
      while (Ptr != End && !llvm::densemap::detail::used(
                               Used, static_cast<unsigned>(&*Ptr - Buckets)))
        ++Ptr;
    } else {
      ...
      const unsigned N = static_cast<unsigned>(End - Buckets);
      unsigned I = static_cast<unsigned>(Ptr - Buckets);
      if (I >= N) {
        Ptr = End;
        return;
      }
      const unsigned NW = llvm::densemap::detail::usedWords(N);
      unsigned W = I >> 5;
