...
// growRelocatable applies it, keeping it out of every instantiated grow().
...
/// Hashes the key a bucket keeps at offset 0. Passing it lets the loops below
/// serve every key type, at an indirect call per entry.
using BucketHasher = unsigned (*)(const void *Key);

/// One instantiation per (KeyT, KeyInfoT) stands in for the whole rehash body
/// it replaces, and is shared by every map with that key, whatever it maps to.
template <typename KeyT, typename KeyInfoT>
unsigned hashKeyAt(const void *Key) {
  return KeyInfoT::getHashValue(*static_cast<const KeyT *>(Key));
}

/// True for a bucket the loops below may move with memcpy. Relocating is a copy
/// at the destination and no destructor at the source, so assignment need not
/// be trivial -- and it is not for a map whose value type is a std::pair.
...
/// free. Requires a bucket that can be relocated by copying its bytes and that
/// keeps its key at offset 0, where \p Hasher reads it.
...
/// The bucket layout growRelocatable needs, in one register.
struct BucketTraits {
  uint32_t Size;
  uint32_t Align;
};

/// growRelocatable's result. NumBuckets repeats the count passed in; taking it
...
/// requirements as rehashRelocatable.
LLVM_ABI TableRef growRelocatable(char *OldBuckets, unsigned OldNumBuckets,
...
protected:
  ...
  // Any hash serves, since the shared loops take it as a function pointer. A
  // bucket keeps its key at offset 0, where hashKeyAt reads it.
  static constexpr bool canShareRehash() {
    return llvm::densemap::detail::isRelocatableBucket<BucketT>;
  }
  ...
  LLVM_ATTRIBUTE_NOINLINE void moveFrom(DerivedT &Other) {
    ...
    if constexpr (canShareRehash()) {
      llvm::densemap::detail::rehashRelocatable(
          ...
          llvm::densemap::detail::hashKeyAt<KeyT, KeyInfoT>);
    } else {
...
private:
  ...
  LLVM_ATTRIBUTE_NOINLINE void grow(unsigned MinNumBuckets) {
    // A plain heap table is just the three fields growRelocatable replaces, so
...
private:
  ...
  void growShared(unsigned MinNumBuckets) {
    auto R = llvm::densemap::detail::growRelocatable(
