...
// True when KeyInfoT hashes the key by its pointer value. An info inheriting
// DenseMapInfo<T *> may hash the pointee, so the tag must name KeyInfoT.
...
inline constexpr bool hashesPointerValue<
    ...
    std::enable_if_t<
        ...
        std::is_same_v<typename KeyInfoT::PointerValueHash, KeyInfoT>>> = true;
