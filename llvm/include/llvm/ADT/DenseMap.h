...
public:
  ...
  /// surviving bucket whose contents were shifted by the backward-shift
  /// pass.  Callers holding external pointers into the bucket array can
  /// use this to update them surgically without scanning the whole map.
...
private:
  ...
  BucketT *findBucketForInsertion(const LookupKeyT &Lookup,
    ...
    // Linear probing with backward-shift deletion keeps every chain compact
    // and bounded by the table's empty-bucket count, so no tombstone-driven
    // resize is needed.
  ...
  /// Backward-shift deletion for linear probing (Knuth TAOCP 6.4 Algorithm R).
  /// The value at \p TheBucket has been destroyed by the caller; this routine
  /// closes the resulting gap by shifting every following live entry that
  /// would otherwise become unreachable, then marks the final gap as empty.
  ///
  /// Note: unlike the previous tombstone-based deletion, this invalidates
  /// every iterator into the table — backshift moves entries that were
  /// pointed at by other iterators.  `incrementEpoch` makes the misuse
  /// trip debug-epoch assertions on the next iterator dereference.
