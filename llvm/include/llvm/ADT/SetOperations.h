...
template <class S1Ty, class S2Ty> void set_intersect(S1Ty &S1, const S2Ty &S2) {
  ...
  if constexpr (detail::HasMemberRemoveIf<S1Ty, decltype(Pred)>) {
    ...
    // Collect the keys to erase before mutating S1: a DenseMap-style erase
    // can relocate other entries to close the gap, which would invalidate
    // any iterator we held across the call.
