...
void ValueHandleBase::RemoveFromUseList() {
  ...
  *PrevPtr = Next;
  ...
  if (Handles.isPointerIntoBucketsArray(PrevPtr)) {
    ...
    // A relocating DenseMap erase can move other surviving entries, which
    // invalidates the PrevP pointers that point into the bucket array.
    // Re-point every remaining list head to its current bucket.
    for (auto I = Handles.begin(), E = Handles.end(); I != E; ++I)
      I->second->setPrevPtr(&I->second);
  }
}
