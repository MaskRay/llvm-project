...
SmallVector<Metadata *> ReplaceableUses::getAllArgListUsers() {
  ...
  for (const UseEntry &U : getUsesInOrder()) {
    if (U.Owner.isNull())
      continue;
...
ReplaceableUses::getAllDbgVariableRecordUsers() {
  ...
  SmallVector<UseEntry, 8> Uses = getUsesInOrder();
  for (const UseEntry &U : reverse(Uses)) {
    if (U.Owner.isNull())
      continue;
...
void ReplaceableUses::SalvageDebugInfo(const Constant &C) {
  ...
  // poison below
  for (const UseEntry &U : MD->getUsesInOrder()) {
