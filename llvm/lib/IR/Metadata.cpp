...
void ReplaceableUses::replaceAllUsesWith(Metadata *MD) {
  ...
  // Copy out uses since UseList will get touched below. Uses are visited
  // oldest first and each callback drops its own entry, so put the oldest at
  // the back where findRef looks first.
  SmallVector<UseEntry, 8> Uses(UseList.begin(), UseList.end());
  std::reverse(UseList.begin(), UseList.end());
  for (const UseEntry &U : Uses) {
