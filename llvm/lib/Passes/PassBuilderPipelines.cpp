...
PipelineTuningOptions::PipelineTuningOptions()
    : PipelineTuningOptions(PassesOptions::Global) {}

PipelineTuningOptions::PipelineTuningOptions(const PassesOptions &Opts)
    : PassesOpts(&Opts) {
  LoopInterleaving = true;
  LoopVectorization = true;
  SLPVectorization = false;
  LoopUnrolling = true;
  LoopInterchange = Opts.LoopInterchange;
  LoopFusion = false;
  ...
  UnifiedLTO = false;
  MergeFunctions = Opts.MergeFunctions;
  InlinerThreshold = -1;
  EagerlyInvalidateAnalyses = Opts.EagerlyInvalidateAnalyses;
  DevirtualizeSpeculatively = Opts.DevirtualizeSpeculatively;
}
...
PassBuilder::buildO1FunctionSimplificationPipeline(OptimizationLevel Level,
  ...
  if (PTO.PassesOpts->EnableLoopFlatten)
...
PassBuilder::buildFunctionSimplificationPipeline(OptimizationLevel Level,
  ...
  // Hoisting of scalars and load expressions.
  if (PTO.PassesOpts->EnableGVNHoist)
    FPM.addPass(GVNHoistPass());

  // Global value numbering based sinking.
  if (PTO.PassesOpts->EnableGVNSink) {
  ...
  if (PTO.PassesOpts->EnableJumpTableToSwitch)
  ...
  if (PTO.PassesOpts->EnableConstraintElimination)
  ...
  // redo DCE, etc.
  if (PTO.PassesOpts->EnableDFAJumpThreading)
...
void PassBuilder::addPreInlinerPasses(ModulePassManager &MPM,
  ...
  if (PTO.PassesOpts->DisablePreInliner)
    return;
  InlineParams IP;

  IP.DefaultThreshold = PTO.PassesOpts->PreInlineThreshold;
  ...
  IP.OptSizeHintThreshold = PTO.PassesOpts->PreInlineThreshold;
...
void PassBuilder::addPostPGOLoopRotation(ModulePassManager &MPM,
                                         OptimizationLevel Level) {
  if (PTO.PassesOpts->EnablePostPGOLoopRotation) {
...
void PassBuilder::addPGOInstrPasses(ModulePassManager &MPM,
  ...
  if (PTO.PassesOpts->EnableSampledInstr) {
...
PassBuilder::buildInlinerPipeline(OptimizationLevel Level,
  ...
  if (PGOOpt)
    IP.EnableDeferral = PTO.PassesOpts->EnablePGOInlineDeferral;

  ModuleInlinerWrapperPass MIWP(
      IP, PTO.PassesOpts->PerformMandatoryInliningsFirst,
      InlineContext{Phase, InlinePass::CGSCCInliner},
      PTO.PassesOpts->UseInlineAdvisor, MaxDevirtIterations);

  // Require the GlobalsAA analysis for the module so we can query it within
  // the CGSCC pipeline.
  if (PTO.PassesOpts->EnableGlobalAnalyses) {
  ...
  if (PTO.PassesOpts->AttributorRun & AttributorRunOption::CGSCC)
    MainCGPipeline.addPass(AttributorCGSCCPass());
  else if (PTO.PassesOpts->AttributorRun & AttributorRunOption::CGSCC_LIGHT)
...
PassBuilder::buildModuleInlinerPipeline(OptimizationLevel Level,
  ...
  MPM.addPass(ModuleInlinerPass(IP, PTO.PassesOpts->UseInlineAdvisor, Phase));
...
PassBuilder::buildModuleSimplificationPipeline(OptimizationLevel Level,
  ...
  bool LoadSampleProfile =
      HasSampleProfile &&
      !(PTO.PassesOpts->FlattenedProfileUsed && isThinLTOPostLink(Phase));
  ...
  if (PTO.PassesOpts->AttributorRun & AttributorRunOption::MODULE)
    MPM.addPass(AttributorPass());
  else if (PTO.PassesOpts->AttributorRun & AttributorRunOption::MODULE_LIGHT)
  ...
  assert((PTO.PassesOpts->InstrumentColdFuncOnlyPath.empty() ||
  ...
  const bool IsColdFuncOnlyInstrGen =
      ...
      !PTO.PassesOpts->InstrumentColdFuncOnlyPath.empty();
  ...
  if (IsPGOInstrGen || IsPGOInstrUse) {
    ...
    addPGOInstrPasses(MPM, Level, /* RunProfileGen */ true, /* IsCS */ false,
                      ...
                      PTO.PassesOpts->InstrumentColdFuncOnlyPath,
  ...
  if (IsPGOPreLink && PGOOpt->CSAction == PGOOptions::CSIRInstr)
    MPM.addPass(PGOInstrumentationGenCreateVar(
        PGOOpt->CSProfileGenFile, PTO.PassesOpts->EnableSampledInstr));
  ...
  if (PTO.PassesOpts->EnableModuleInliner)
...
void PassBuilder::addVectorPasses(OptimizationLevel Level,
  ...
  if (isFullLTOPostLink(LTOPhase)) {
    ...
    if (PTO.PassesOpts->EnableUnrollAndJam && PTO.LoopUnrolling)
      FPM.addPass(createFunctionToLoopPassAdaptor(
  ...
  if (Level > OptimizationLevel::O1 && PTO.PassesOpts->ExtraVectorizerPasses) {
  ...
  if (PTO.SLPVectorization) {
    ...
    if (Level >= OptimizationLevel::O2 &&
        PTO.PassesOpts->ExtraVectorizerPasses) {
...
PassBuilder::buildModuleOptimizationPipeline(OptimizationLevel Level,
  ...
  // large bodies.
  if (PTO.PassesOpts->RunPartialInlining)
  ...
  if (PTO.PassesOpts->UseLoopVersioningLICM) {
    OptimizePM.addPass(
  ...
  if (PTO.PassesOpts->EnableMatrix) {
  ...
  if (PTO.PassesOpts->EnableCHR && Level == OptimizationLevel::O3)
  ...
  if (PTO.PassesOpts->EnableMergeICmps)
  ...
  if (PTO.PassesOpts->EnableInstrumentor)
    MPM.addPass(InstrumentorPass(FS));

  // Split out cold code. Splitting is done late to avoid hiding context from
  // other optimizations and inadvertently regressing performance. The tradeoff
  // is that this has a higher code size cost than splitting early.
  if (PTO.PassesOpts->EnableHotColdSplit && !isLTOPreLink(LTOPhase))
  ...
  if (PTO.DevirtualizeSpeculatively && LTOPhase == ThinOrFullLTOPhase::None) {
    ...
    if (PTO.PassesOpts->EnableModuleInliner) {
      MPM.addPass(ModuleInlinerPass(::getInlineParamsFromOptLevel(Level),
                                    PTO.PassesOpts->UseInlineAdvisor,
...
PassBuilder::buildPerModuleDefaultPipeline(OptimizationLevel Level,
  ...
  if (PTO.PassesOpts->TriggerCrash)
...
PassBuilder::buildLTODefaultPipeline(OptimizationLevel Level,
  ...
  LoopPassManager LPM;
  if (PTO.PassesOpts->EnableLoopFlatten && Level >= OptimizationLevel::O2)
  ...
  if (PTO.PassesOpts->EnableHotColdSplit)
    MPM.addPass(HotColdSplittingPass());
