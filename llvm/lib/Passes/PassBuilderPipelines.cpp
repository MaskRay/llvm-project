...
static bool attributorRuns(const PassesOptions &Opts,
                           AttributorRunOption Kind) {
  return unsigned(Opts.AttributorRun) & unsigned(Kind);
}
...
PipelineTuningOptions::PipelineTuningOptions() {
  ...
  LoopUnrolling = true;
  const PassesOptions &Opts = PassesOptions::current();
...
PassBuilder::buildFunctionSimplificationPipeline(OptimizationLevel Level,
  ...
  // Hoisting of scalars and load expressions.
  if (Opts.EnableGVNHoist)
    FPM.addPass(GVNHoistPass());

  // Global value numbering based sinking.
  if (Opts.EnableGVNSink) {
  ...
  if (Opts.EnableJumpTableToSwitch)
  ...
  if (Opts.EnableConstraintElimination)
  ...
  // redo DCE, etc.
  if (Opts.EnableDFAJumpThreading)
...
void PassBuilder::addPreInlinerPasses(ModulePassManager &MPM,
  ...
  if (Opts.DisablePreInliner)
    return;
  InlineParams IP;

  IP.DefaultThreshold = Opts.PreInlineThreshold;
  ...
  IP.OptSizeHintThreshold = Opts.PreInlineThreshold;
...
void PassBuilder::addPostPGOLoopRotation(ModulePassManager &MPM,
                                         OptimizationLevel Level) {
  if (Opts.EnablePostPGOLoopRotation) {
...
PassBuilder::buildInlinerPipeline(OptimizationLevel Level,
  ...
  if (PGOOpt)
    IP.EnableDeferral = Opts.EnablePGOInlineDeferral;

  ModuleInlinerWrapperPass MIWP(IP, Opts.PerformMandatoryInliningsFirst,
                                InlineContext{Phase, InlinePass::CGSCCInliner},
                                Opts.UseInlineAdvisor, MaxDevirtIterations);

  // Require the GlobalsAA analysis for the module so we can query it within
  // the CGSCC pipeline.
  if (Opts.EnableGlobalAnalyses) {
  ...
  if (attributorRuns(Opts, AttributorRunOption::CGSCC))
    MainCGPipeline.addPass(AttributorCGSCCPass());
  else if (attributorRuns(Opts, AttributorRunOption::CGSCC_LIGHT))
...
PassBuilder::buildModuleInlinerPipeline(OptimizationLevel Level,
  ...
  MPM.addPass(ModuleInlinerPass(IP, Opts.UseInlineAdvisor, Phase));
...
PassBuilder::buildModuleSimplificationPipeline(OptimizationLevel Level,
  ...
  bool LoadSampleProfile = HasSampleProfile && !(Opts.FlattenedProfileUsed &&
  ...
  if (attributorRuns(Opts, AttributorRunOption::MODULE))
    MPM.addPass(AttributorPass());
  else if (attributorRuns(Opts, AttributorRunOption::MODULE_LIGHT))
  ...
  assert((Opts.InstrumentColdFuncOnlyPath.empty() ||
  ...
  const bool IsColdFuncOnlyInstrGen = PGOInstrumentColdFunctionOnly &&
                                      IsPGOPreLink &&
                                      !Opts.InstrumentColdFuncOnlyPath.empty();
  ...
  if (IsPGOInstrGen || IsPGOInstrUse) {
    ...
    addPGOInstrPasses(MPM, Level, /* RunProfileGen */ true, /* IsCS */ false,
                      ...
                      Opts.InstrumentColdFuncOnlyPath,
...
void PassBuilder::addVectorPasses(OptimizationLevel Level,
  ...
  if (isFullLTOPostLink(LTOPhase)) {
    ...
    if (Opts.EnableUnrollAndJam && PTO.LoopUnrolling)
      FPM.addPass(createFunctionToLoopPassAdaptor(
  ...
  if (Level > OptimizationLevel::O1 && Opts.ExtraVectorizerPasses) {
  ...
  if (PTO.SLPVectorization) {
    ...
    if (Level >= OptimizationLevel::O2 && Opts.ExtraVectorizerPasses) {
...
PassBuilder::buildModuleOptimizationPipeline(OptimizationLevel Level,
  ...
  if (Opts.EnableCHR && Level == OptimizationLevel::O3)
  ...
  if (Opts.EnableInstrumentor)
    MPM.addPass(InstrumentorPass(FS));

  // Split out cold code. Splitting is done late to avoid hiding context from
  // other optimizations and inadvertently regressing performance. The tradeoff
  // is that this has a higher code size cost than splitting early.
  if (Opts.EnableHotColdSplit && !isLTOPreLink(LTOPhase))
...
PassBuilder::buildLTODefaultPipeline(OptimizationLevel Level,
  ...
  LoopPassManager LPM;
  if (Opts.EnableLoopFlatten && Level >= OptimizationLevel::O2)
