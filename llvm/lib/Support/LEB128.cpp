...
void llvm::encodeCLeb128(uint64_t x, raw_ostream &os) {
  // Fast path for n == 1
  if (x < 128) {
    os.write((x << 1) | 1);
    return;
...
template <int n>
static inline uint64_t getCLeb128Case(const uint8_t *&p, uint8_t byte) {
  uint64_t val = byte >> n;
  int shift = 8 - n;
  for (int i = 1; i < n; ++i) {
...
static uint64_t getUCLeb128Impl(const uint8_t *&p, const uint8_t *end) {
  ...
  switch (n) {
  case 1:
    return getCLeb128Case<1>(p, b0);
  case 2:
    return getCLeb128Case<2>(p, b0);
  case 3:
    return getCLeb128Case<3>(p, b0);
  case 4:
    return getCLeb128Case<4>(p, b0);
  case 5:
    return getCLeb128Case<5>(p, b0);
  case 6:
    return getCLeb128Case<6>(p, b0);
  case 7:
    return getCLeb128Case<7>(p, b0);
  case 8:
    return getCLeb128Case<8>(p, b0);
  default:
    // 9 bytes: 00000000 xxxxxxxx ...
    p += 9;
