...
static size_t tableBytes(size_t BucketSize, unsigned Num) {
  return BucketSize * Num + usedWords(Num) * sizeof(UsedT);
}
...
void *densemap::detail::growRelocatable(void *OldBuckets, const UsedT *OldUsed,
  ...
  void *Storage = allocate_buffer(tableBytes(BucketSize, NewNumBuckets), Align);
  ...
  if (OldNumBuckets) {
    ...
    if (FreeOld)
      deallocate_buffer(OldBuckets, tableBytes(BucketSize, OldNumBuckets),
                        Align);
  }
  return Storage;
}
