...
static bool rebuildLoopAfterUnswitch(Loop &L, DominatorTree &DT, LoopInfo &LI,
  ...
  bool IsStillLoop = !is_contained(Removed, &L);
  for (Loop *ChildL : Children)
    if (!is_contained(Removed, ChildL) && ChildL->getParentLoop() != &L)
      HoistedLoops.push_back(ChildL);

  for (Loop *RemovedL : Removed) {
    assert((RemovedL == &L || is_contained(Children, RemovedL)) &&
           "Unswitching can only remove loops from the current nest!");
    // markLoopAsDeleted for L is triggered by the caller (postUnswitch).
    if (RemovedL == &L)
      continue;
    LoopUpdater.markLoopAsDeleted(*RemovedL, RemovedL->getName());
    LI.destroy(RemovedL);
  }
  if (SE && !Removed.empty())
    SE->forgetBlockAndLoopDispositions();
