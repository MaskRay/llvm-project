...
bool RegAllocFastImpl::lowerTiedUse(MachineInstr &MI, MachineOperand &MO,
  ...
  if (!LR.PhysReg) {
    // DefReg may be unallocatable (e.g. the base pointer, tied to an inline
    // asm operand). An early-clobber def is written before the uses are read,
    // so it must not share a register with another operand reading the value.
    bool MustCopy = !MRI->isAllocatable(DefReg) ||
