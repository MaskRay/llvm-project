...
void ELFWriter::writeSectionHeaderEntry(
  ...
  if (TO && TO->CompactShdr) {
    ...
    uint8_t presence = (Type != ELF::SHT_PROGBITS ? 1 : 0) | (Flags ? 2 : 0) |
                       ...
                       (EntrySize ? 128 : 0);
    W.OS << presence;
    encodeUCLeb128(Name, W.OS);   // sh_name
    encodeUCLeb128(Offset, W.OS); // sh_offset
    if (Type != ELF::SHT_PROGBITS)
      encodeUCLeb128(Type, W.OS); // sh_type
    if (Flags)
      encodeUCLeb128(Flags, W.OS); // sh_flags
    if (Address)
      encodeUCLeb128(Address, W.OS); // sh_addr
    if (Size)
      encodeUCLeb128(Size, W.OS); // sh_size
    if (Link)
      encodeUCLeb128(Link, W.OS); // sh_link
    if (Info)
      encodeUCLeb128(Info, W.OS); // sh_info
    if (align)
      encodeUCLeb128(align, W.OS); // sh_addralign
    if (EntrySize)
      encodeUCLeb128(EntrySize, W.OS); // sh_entsize
    return;
  }
