...
void ELFWriter::writeSectionHeaderEntry(
  ...
  if (TO && TO->CompactShdr) {
    ...
    uint8_t presence = (Type != ELF::SHT_PROGBITS ? 1 : 0) | (Flags ? 2 : 0) |
                       ...
                       (EntrySize ? 128 : 0);
    W.OS << presence;
    encodeCLeb128(Name, W.OS);   // sh_name
    encodeCLeb128(Offset, W.OS); // sh_offset
    if (Type != ELF::SHT_PROGBITS)
      encodeCLeb128(Type, W.OS); // sh_type
    if (Flags)
      encodeCLeb128(Flags, W.OS); // sh_flags
    if (Address)
      encodeCLeb128(Address, W.OS); // sh_addr
    if (Size)
      encodeCLeb128(Size, W.OS); // sh_size
    if (Link)
      encodeCLeb128(Link, W.OS); // sh_link
    if (Info)
      encodeCLeb128(Info, W.OS); // sh_info
    if (align)
      encodeCLeb128(align, W.OS); // sh_addralign
    if (EntrySize)
      encodeCLeb128(EntrySize, W.OS); // sh_entsize
    return;
  }
