...
MCSectionELF *ELFWriter::createRelocationSection(MCContext &Ctx,
  ...
  if (TO && TO->RELLEB) {
    MCSectionELF *RelaSection =
        Ctx.createELFRelSection(".relleb" + SectionName, ELF::SHT_RELLEB, Flags,
...
template <class uint>
static void encodeRELLEB(ArrayRef<ELFRelocationEntry> Relocs,
...
void ELFWriter::writeRelocations(const MCAssembler &Asm, MCSectionELF &RelSec) {
  ...
  if (TO && TO->RELLEB) {
    if (is64Bit())
      encodeRELLEB<uint64_t>(Relocs, os);
    else
      encodeRELLEB<uint32_t>(Relocs, os);
  } else {
    for (unsigned i = 0, e = Relocs.size(); i != e; ++i) {
      const ELFRelocationEntry &Entry = Relocs[e - i - 1];
