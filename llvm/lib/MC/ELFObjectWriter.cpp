...
static void encodeCrel(ArrayRef<ELFRelocationEntry> Relocs, raw_ostream &os) {
  ...
  encodeVU128(Relocs.size() * 4 + Shift, os);
  for (const ELFRelocationEntry &Entry : Relocs) {
    ...
    uint64_t B = (DeltaOffset << 3) + (Symidx != CurSymidx) +
                 (Type != Entry.Type ? 2 : 0) +
                 (Addend != Entry.Addend ? 4 : 0);
    encodeVU128(B, os);
    // Delta symidx/type/addend members (SLEB128).
    if (B & 1) {
      encodeVS128(static_cast<int32_t>(CurSymidx - Symidx), os);
      Symidx = CurSymidx;
    }
    if (B & 2) {
      encodeVS128(static_cast<int32_t>(Entry.Type - Type), os);
      Type = Entry.Type;
    }
    if (B & 4) {
      encodeVS128(std::make_signed_t<uint>(Entry.Addend - Addend), os);
      Addend = Entry.Addend;
    }
  }
}
