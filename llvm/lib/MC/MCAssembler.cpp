...
bool MCAssembler::evaluateFixup(const MCFragment &F, MCFixup &Fixup,
  ...
  if (auto State = getBackend().evaluateFixup(F, Fixup, Target, Value)) {
    ...
    if (Fixup.isPCRel()) {
      ...
      // During relaxation (fused layout+relax pass), the source fragment's
      // offset is already updated but forward reference targets are stale.
      // Add Stretch so that the displacement equals target_old - source_old
      // (same as the old two-pass approach), preventing premature relaxation.
      if (!RecordReloc && Stretch) {
        if (Add && Add->isDefined())
          if (auto *AF = Add->getFragment())
            if (AF->getLayoutOrder() > F.getLayoutOrder())
              Value += Stretch;
        if (Sub && Sub->isDefined())
          if (auto *SF = Sub->getFragment())
            if (SF->getLayoutOrder() > F.getLayoutOrder())
              Value -= Stretch;
      }
