...
public:
  ...
  const MCExpr *symbolRefWithVariant(const MCSymbol *S,
...
const MCExpr *PPCAsmPrinter::symbolRefWithVariant(const MCSymbol *S,
                                                  PPCMCExpr::VariantKind Kind) {
  const MCExpr *Expr = MCSymbolRefExpr::create(S, OutContext);
  if (Kind != PPCMCExpr::VK_None)
    Expr = PPCMCExpr::create(Kind, Expr, OutContext);
  return Expr;
}
...
void PPCAsmPrinter::emitInstruction(const MachineInstr *MI) {
  ...
  case PPC::LWZtoc: {
    ...
    if (PL == PICLevel::SmallPIC && !IsAIX) {
      const MCExpr *Exp = symbolRefWithVariant(MOSymbol, PPCMCExpr::VK_GOT);
  ...
  case PPC::ADDItocL8: {
    ...
    const MCExpr *Exp = MCSymbolRefExpr::create(
        MOSymbol,
        MCSymbolRefExpr::VariantKind(IsAIX ? PPCMCExpr::VK_PPC_L
                                           : PPCMCExpr::VK_PPC_TOC_LO),
        OutContext);
  ...
  case PPC::LDgotTprelL32: {
    ...
    MCSymbol *MOSymbol = getSymbol(GValue);
    const MCExpr *Exp =
        symbolRefWithVariant(MOSymbol, IsPPC64 ? PPCMCExpr::VK_PPC_GOT_TPREL_LO
                                               : PPCMCExpr::VK_PPC_GOT_TPREL);
    TmpInst.getOperand(1) = MCOperand::createExpr(Exp);
    EmitToStreamer(*OutStreamer, TmpInst);
    return;
  }
...
void PPCLinuxAsmPrinter::emitFunctionEntryLabel() {
  ...
  OutStreamer->emitValue(
      MCSymbolRefExpr::create(
          Symbol2, MCSymbolRefExpr::VariantKind(PPCMCExpr::VK_PPC_TOCBASE),
          OutContext),
      8 /*size*/);
