...
struct DenseMapInfo<std::pair<const MCSymbol *, PPCMCExpr::VariantKind>> {
  ...
  static inline TOCKey getEmptyKey() {
    return {nullptr, PPCMCExpr::VariantKind::VK_None};
  }
...
public:
  ...
  MCSymbol *lookUpOrCreateTOCEntry(
      const MCSymbol *Sym, TOCEntryType Type,
      PPCMCExpr::VariantKind Kind = PPCMCExpr::VariantKind::VK_None);
...
void PPCAsmPrinter::emitInstruction(const MachineInstr *MI) {
  ...
  auto symbolRefWithVariant = [&](const MCSymbol *S,
                                  PPCMCExpr::VariantKind Kind) {
    return PPCMCExpr::create(
        Kind, MCSymbolRefExpr::create(S, PPCMCExpr::VK_None, OutContext),
        OutContext);
  };
  ...
  auto getTOCEntryLoadingExprForXCOFF =
      [IsPPC64, getTOCRelocAdjustedExprForXCOFF,
       this](const MCSymbol *MOSymbol, const MCExpr *Expr,
             PPCMCExpr::VariantKind VK =
                 PPCMCExpr::VariantKind::VK_None) -> const MCExpr * {
  ...
  auto GetVKForMO = [&](const MachineOperand &MO) {
    ...
    if (Flag == PPCII::MO_TPREL_FLAG ||
      ...
      if (Model == TLSModel::LocalExec)
        return PPCMCExpr::VK_PPC_AIX_TLSLE;
      if (Model == TLSModel::InitialExec)
        return PPCMCExpr::VK_PPC_AIX_TLSIE;
    ...
    if (Flag == PPCII::MO_TLSGDM_FLAG)
      return PPCMCExpr::VK_PPC_AIX_TLSGDM;
    if (Flag == PPCII::MO_TLSGD_FLAG || Flag == PPCII::MO_GOT_TLSGD_PCREL_FLAG)
      return PPCMCExpr::VK_PPC_AIX_TLSGD;
    ...
    if (Flag == PPCII::MO_TLSLD_FLAG && IsAIX)
      return PPCMCExpr::VK_PPC_AIX_TLSLD;
    if (Flag == PPCII::MO_TLSLDM_FLAG && IsAIX)
      return PPCMCExpr::VK_PPC_AIX_TLSML;
    return PPCMCExpr::VariantKind::VK_None;
  };
  ...
  case PPC::MoveGOTtoLR: {
    ...
    const MCExpr *OffsExpr = MCBinaryExpr::createSub(
        symbolRefWithVariant(
            GOTSymbol, MCSymbolRefExpr::VariantKind(PPCMCExpr::VK_PPC_LOCAL)),
  ...
  case PPC::MovePCtoLR8: {
    ...
    EmitToStreamer(*OutStreamer,
                   MCInstBuilder(PPC::BL)
                       ...
                       .addExpr(symbolRefWithVariant(PICBase)));
    ...
    if (Subtarget->isSecurePlt() && isPositionIndependent() ) {
      ...
      MCSymbol *BaseSymbol = OutContext.getOrCreateSymbol(
          M->getPICLevel() == PICLevel::SmallPIC ? "_GLOBAL_OFFSET_TABLE_"
                                                 : ".LTOC");
      const MCExpr *PB = symbolRefWithVariant(MF->getPICBaseSymbol());

      const MCExpr *DeltaExpr = MCBinaryExpr::createSub(
          symbolRefWithVariant(BaseSymbol, OutContext), PB);
  ...
  case PPC::LWZtoc: {
    ...
    if (PL == PICLevel::SmallPIC && !IsAIX) {
      const MCExpr *Exp =
          MCSymbolRefExpr::create(MOSymbol, PPCMCExpr::VK_GOT, OutContext);
    ...
    const MCExpr *PB = symbolRefWithVariant(
        OutContext.getOrCreateSymbol(Twine(".LTOC")), MCSymbolRefExpr::VK_None);
  ...
  case PPC::LDtoc: {
    ...
    PPCMCExpr::VariantKind VKExpr =
        IsAIX ? PPCMCExpr::VK_None : PPCMCExpr::VK_PPC_TOC;
    const MCExpr *Exp = symbolRefWithVariant(TOCEntry, VKExpr);
  ...
  case PPC::ADDIStocHA: {
    ...
    const MCExpr *Exp = symbolRefWithVariant(MOSymbol, PPCMCExpr::VK_PPC_U);
    TmpInst.getOperand(2) = MCOperand::createExpr(Exp);
    EmitToStreamer(*OutStreamer, TmpInst);
    return;
  }
  case PPC::LWZtocL: {
    ...
    MCSymbol *TOCEntry =
        lookUpOrCreateTOCEntry(MOSymbol, getTOCEntryTypeForMO(MO), VK);
    const MCExpr *Exp = symbolRefWithVariant(TOCEntry, PPCMCExpr::VK_PPC_L);
    TmpInst.getOperand(1) = MCOperand::createExpr(Exp);
    EmitToStreamer(*OutStreamer, TmpInst);
    return;
  }
  case PPC::ADDIStocHA8: {
    ...
    VK = IsAIX ? PPCMCExpr::VK_PPC_U : PPCMCExpr::VK_PPC_TOC_HA;

    const MCExpr *Exp = symbolRefWithVariant(MOSymbol, VK);
  ...
  case PPC::LDtocL: {
    ...
    VK = IsAIX ? PPCMCExpr::VK_PPC_L : PPCMCExpr::VK_PPC_TOC_LO;
    const MCExpr *Exp = symbolRefWithVariant(MOSymbol, VK);
    TmpInst.getOperand(1) = MCOperand::createExpr(Exp);
    EmitToStreamer(*OutStreamer, TmpInst);
    return;
  ...
  case PPC::ADDItocL8: {
    ...
    const MCExpr *Exp = MCSymbolRefExpr::create(
        MOSymbol, IsAIX ? PPCMCExpr::VK_PPC_L : PPCMCExpr::VK_PPC_TOC_LO,
        OutContext);
  ...
  case PPC::ADDISgotTprelHA: {
    ...
    const MCExpr *SymGotTprel =
        symbolRefWithVariant(MOSymbol, PPCMCExpr::VK_PPC_GOT_TPREL_HA);
  ...
  case PPC::LDgotTprelL32: {
    ...
    const MCExpr *Exp = MCSymbolRefExpr::create(
        MOSymbol,
        IsPPC64 ? PPCMCExpr::VK_PPC_GOT_TPREL_LO : PPCMCExpr::VK_PPC_GOT_TPREL,
        OutContext);
    TmpInst.getOperand(1) = MCOperand::createExpr(Exp);
    EmitToStreamer(*OutStreamer, TmpInst);
    return;
  ...
  case PPC::ADDIStlsgdHA: {
    ...
    const MCExpr *SymGotTlsGD =
        symbolRefWithVariant(MOSymbol, PPCMCExpr::VK_PPC_GOT_TLSGD_HA);
    EmitToStreamer(*OutStreamer, MCInstBuilder(PPC::ADDIS8)
                                 .addReg(MI->getOperand(0).getReg())
                                 .addReg(MI->getOperand(1).getReg())
                                 .addExpr(SymGotTlsGD));
  ...
  case PPC::ADDItlsgdL32: {
    ...
    MCSymbol *MOSymbol = getSymbol(GValue);
    const MCExpr *SymGotTlsGD =
        symbolRefWithVariant(MOSymbol, IsPPC64 ? PPCMCExpr::VK_PPC_GOT_TLSGD_LO
                                               : PPCMCExpr::VK_PPC_GOT_TLSGD);
    EmitToStreamer(*OutStreamer,
  ...
  case PPC::ADDIStlsldHA: {
    ...
    const MCExpr *SymGotTlsLD =
        symbolRefWithVariant(MOSymbol, PPCMCExpr::VK_PPC_GOT_TLSLD_HA);
    EmitToStreamer(*OutStreamer, MCInstBuilder(PPC::ADDIS8)
                                 .addReg(MI->getOperand(0).getReg())
                                 .addReg(MI->getOperand(1).getReg())
                                 .addExpr(SymGotTlsLD));
  ...
  case PPC::ADDItlsldL32: {
    ...
    MCSymbol *MOSymbol = getSymbol(GValue);
    const MCExpr *SymGotTlsLD =
        symbolRefWithVariant(MOSymbol, IsPPC64 ? PPCMCExpr::VK_PPC_GOT_TLSLD_LO
                                               : PPCMCExpr::VK_PPC_GOT_TLSLD);
    EmitToStreamer(*OutStreamer,
                   MCInstBuilder(IsPPC64 ? PPC::ADDI8 : PPC::ADDI)
                       .addReg(MI->getOperand(0).getReg())
                       .addReg(MI->getOperand(1).getReg())
  ...
  case PPC::ADDISdtprelHA32: {
    ...
    const MCExpr *SymDtprel =
        symbolRefWithVariant(MOSymbol, PPCMCExpr::VK_PPC_DTPREL_HA);
    EmitToStreamer(
        *OutStreamer,
  ...
  case PPC::PADDIdtprel: {
    ...
    MCSymbol *MOSymbol = getSymbol(GValue);
    const MCExpr *SymDtprel =
        symbolRefWithVariant(MOSymbol, PPCMCExpr::VK_DTPREL);
  ...
  case PPC::ADDIdtprelL32: {
    ...
    MCSymbol *MOSymbol = getSymbol(GValue);
    const MCExpr *SymDtprel =
        symbolRefWithVariant(MOSymbol, PPCMCExpr::VK_PPC_DTPREL_LO);
    EmitToStreamer(*OutStreamer,
                   MCInstBuilder(IsPPC64 ? PPC::ADDI8 : PPC::ADDI)
                       .addReg(MI->getOperand(0).getReg())
                       .addReg(MI->getOperand(1).getReg())
...
PPCAsmPrinter::getAdjustedFasterLocalExpr(const MachineOperand &MO,
  ...
  const MCExpr *Expr = MCSymbolRefExpr::create(
      ...
      MCSymbolRefExpr::VariantKind(Model == TLSModel::LocalExec
                                       ? PPCMCExpr::VK_PPC_AIX_TLSLE
                                       : PPCMCExpr::VK_PPC_AIX_TLSLD),
      OutContext);
...
void PPCLinuxAsmPrinter::emitFunctionEntryLabel() {
  ...
  OutStreamer->emitValue(
      MCSymbolRefExpr::create(Symbol2,
                              PPCMCExpr::VariantKind(PPCMCExpr::VK_PPC_TOCBASE),
                              OutContext),
      8 /*size*/);
...
void PPCAIXAsmPrinter::emitEndOfAsmFile(Module &M) {
  ...
  for (auto &I : TOC) {
    ...
    // with a dot.
    if (I.first.second == PPCMCExpr::VK_PPC_AIX_TLSGDM ||
        (Subtarget->hasAIXShLibTLSModelOpt() &&
         I.first.second == PPCMCExpr::VK_PPC_AIX_TLSLD)) {
