...
struct DenseMapInfo<std::pair<const MCSymbol *, PPCMCExpr::VariantKind>> {
  ...
  static inline TOCKey getEmptyKey() {
    return {nullptr, PPCMCExpr::VK_None};
  }
...
void PPCAsmPrinter::emitInstruction(const MachineInstr *MI) {
  ...
  auto getTOCEntryLoadingExprForXCOFF =
      [IsPPC64, getTOCRelocAdjustedExprForXCOFF,
       this](const MCSymbol *MOSymbol, const MCExpr *Expr,
             PPCMCExpr::VariantKind VK =
                 PPCMCExpr::VK_None) -> const MCExpr * {
  ...
  case PPC::ADDIStocHA: {
    ...
    const MCExpr *Exp = symbolRefWithVariant(MOSymbol, PPCMCExpr::VK_U);
    TmpInst.getOperand(2) = MCOperand::createExpr(Exp);
    EmitToStreamer(*OutStreamer, TmpInst);
    return;
  }
  case PPC::LWZtocL: {
    ...
    MCSymbol *TOCEntry =
        lookUpOrCreateTOCEntry(MOSymbol, getTOCEntryTypeForMO(MO), VK);
    const MCExpr *Exp = symbolRefWithVariant(TOCEntry, PPCMCExpr::VK_L);
    TmpInst.getOperand(1) = MCOperand::createExpr(Exp);
    EmitToStreamer(*OutStreamer, TmpInst);
    return;
  }
  ...
  case PPC::ADDISgotTprelHA: {
    ...
    const MCExpr *SymGotTprel =
        symbolRefWithVariant(MOSymbol, PPCMCExpr::VK_GOT_TPREL_HA);
  ...
  case PPC::LDgotTprelL32: {
    ...
    MCSymbol *MOSymbol = getSymbol(GValue);
    const MCExpr *Exp =
        symbolRefWithVariant(MOSymbol, IsPPC64 ? PPCMCExpr::VK_GOT_TPREL_LO
  ...
  case PPC::ADDIStlsgdHA: {
    ...
    const MCExpr *SymGotTlsGD =
        symbolRefWithVariant(MOSymbol, PPCMCExpr::VK_GOT_TLSGD_HA);
    EmitToStreamer(*OutStreamer, MCInstBuilder(PPC::ADDIS8)
                                 .addReg(MI->getOperand(0).getReg())
                                 .addReg(MI->getOperand(1).getReg())
                                 .addExpr(SymGotTlsGD));
  ...
  case PPC::ADDItlsgdL32: {
    ...
    MCSymbol *MOSymbol = getSymbol(GValue);
    const MCExpr *SymGotTlsGD =
        symbolRefWithVariant(MOSymbol, IsPPC64 ? PPCMCExpr::VK_GOT_TLSGD_LO
  ...
  case PPC::ADDIStlsldHA: {
    ...
    const MCExpr *SymGotTlsLD =
        symbolRefWithVariant(MOSymbol, PPCMCExpr::VK_GOT_TLSLD_HA);
    EmitToStreamer(*OutStreamer, MCInstBuilder(PPC::ADDIS8)
                                 .addReg(MI->getOperand(0).getReg())
                                 .addReg(MI->getOperand(1).getReg())
                                 .addExpr(SymGotTlsLD));
  ...
  case PPC::ADDItlsldL32: {
    ...
    MCSymbol *MOSymbol = getSymbol(GValue);
    const MCExpr *SymGotTlsLD =
        symbolRefWithVariant(MOSymbol, IsPPC64 ? PPCMCExpr::VK_GOT_TLSLD_LO
  ...
  case PPC::ADDISdtprelHA32: {
    ...
    const MCExpr *SymDtprel =
        symbolRefWithVariant(MOSymbol, PPCMCExpr::VK_DTPREL_HA);
    EmitToStreamer(
        *OutStreamer,
  ...
  case PPC::ADDIdtprelL32: {
    ...
    MCSymbol *MOSymbol = getSymbol(GValue);
    const MCExpr *SymDtprel =
        symbolRefWithVariant(MOSymbol, PPCMCExpr::VK_DTPREL_LO);
    EmitToStreamer(*OutStreamer,
                   MCInstBuilder(IsPPC64 ? PPC::ADDI8 : PPC::ADDI)
                       .addReg(MI->getOperand(0).getReg())
                       .addReg(MI->getOperand(1).getReg())
