...
public:
  ...
  void emitTCEntry(const MCSymbol &S, PPCMCExpr::VariantKind Kind) override {
    if (const MCSymbolXCOFF *XSym = dyn_cast<MCSymbolXCOFF>(&S)) {
      ...
      // 32-bit mode.
      if (Kind == PPCMCExpr::VK_PPC_AIX_TLSGD ||
          Kind == PPCMCExpr::VK_PPC_AIX_TLSGDM ||
          Kind == PPCMCExpr::VK_PPC_AIX_TLSIE ||
          Kind == PPCMCExpr::VK_PPC_AIX_TLSLE ||
          Kind == PPCMCExpr::VK_PPC_AIX_TLSLD ||
          Kind == PPCMCExpr::VK_PPC_AIX_TLSML)
