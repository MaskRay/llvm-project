...
PPCMCCodeEmitter::getDispRI34PCRelEncoding(const MCInst &MI, unsigned OpNo,
  ...
  case MCExpr::SymbolRef: {
    ...
    assert((getVariantKind(SRE) == PPCMCExpr::VK_PCREL ||
            getVariantKind(SRE) == PPCMCExpr::VK_PPC_GOT_PCREL ||
            getVariantKind(SRE) == PPCMCExpr::VK_PPC_GOT_TLSGD_PCREL ||
            getVariantKind(SRE) == PPCMCExpr::VK_PPC_GOT_TLSLD_PCREL ||
            getVariantKind(SRE) == PPCMCExpr::VK_PPC_GOT_TPREL_PCREL) &&
...
unsigned PPCMCCodeEmitter::getTLSRegEncoding(const MCInst &MI, unsigned OpNo,
  ...
  const MCExpr *Expr = MO.getExpr();
  const MCSymbolRefExpr *SRE = cast<MCSymbolRefExpr>(Expr);
  bool IsPCRel = getVariantKind(SRE) == PPCMCExpr::VK_PPC_TLS_PCREL;
