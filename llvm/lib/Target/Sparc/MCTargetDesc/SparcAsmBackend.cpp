...
public:
  ...
  void printExpr(raw_ostream &OS, const MCSpecifierExpr &Expr) const override;
...
void SparcAsmBackend::printExpr(raw_ostream &OS,
                                const MCSpecifierExpr &Expr) const {
  StringRef S = SparcMCExpr::getSpecifierName(Expr.getSpecifier());
  if (!S.empty())
    OS << '%' << S << '(';
  Expr.getSubExpr()->print(OS, getContext().getAsmInfo());
  if (!S.empty())
    OS << ')';
}
