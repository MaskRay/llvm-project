...
void X86MachObjectWriter::RecordX86_64Relocation(
  ...
  if (Target.isAbsolute()) { // constant
    ...
    if (getVariantKind(Target.getSymA()) != X86MCExpr::VK_None) {
    ...
    auto Modifier = getVariantKind(Target.getSymA());
    if (IsPCRel) {
      if (IsRIPRel) {
        if (Modifier == X86MCExpr::VK_GOTPCREL) {
        ...
        } else if (Modifier == X86MCExpr::VK_TLVP) {
          Type = MachO::X86_64_RELOC_TLV;
        } else if (Modifier != X86MCExpr::VK_None) {
          Asm.getContext().reportError(
      ...
      if (Modifier == X86MCExpr::VK_GOT) {
        Type = MachO::X86_64_RELOC_GOT;
      } else if (Modifier == X86MCExpr::VK_GOTPCREL) {
...
void X86MachObjectWriter::recordTLVPRelocation(MachObjectWriter *Writer,
  ...
  assert(getVariantKind(SymA) == X86MCExpr::VK_TLVP && !is64Bit() &&
...
void X86MachObjectWriter::RecordX86Relocation(MachObjectWriter *Writer,
  ...
  if (Target.getSymA() &&
      getVariantKind(Target.getSymA()) == X86MCExpr::VK_TLVP) {
