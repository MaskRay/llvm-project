...
// Emits the struct \p S names: its declaration under OPTIONS_STRUCT_DECL, and
// its statics, apply() and parse() under OPTIONS_STRUCT_DEFS, which also guards
// the tables they use.
static void emitOptionsStruct(const Record &S, ArrayRef<const Record *> Groups,
  ...
  for (const Record *R : Opts) {
    ...
    if (!is_contained({"Flag", "Joined", "Separate", "Input", "Unknown"}, Kind))
      PrintFatalError(R->getLoc(), "an option beside an OptionsStruct must be "
                                   "a Flag, Joined or Separate");
  }
  ...
  OS << "  /// The struct's slot in an OptionsRegistry; -1 until registered.\n"
        ...
        "appending every\n"
        "  /// other argument to \\p Rest in order. On error, writes a line "
        "to \\p Errs\n"
        "  /// and returns false.\n"
...
/// Emits the tables for parsing the options a .td file describes, and the
/// struct of its OptionsStruct if any.
