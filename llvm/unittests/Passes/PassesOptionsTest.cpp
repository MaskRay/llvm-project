...
TEST(PassesOptionsTest, Parse) {
  ...
  EXPECT_EQ(Err, "error: invalid value '' in '-enable-chr='\n");
}
...
TEST(PassesOptionsTest, PerPipeline) {
  ...
  EXPECT_TRUE(StringRef(Pipeline(A)).contains("MergeFunctionsPass"));
  EXPECT_FALSE(StringRef(Pipeline(B)).contains("ModuleInlinerPass"));
  EXPECT_FALSE(StringRef(Pipeline(B)).contains("MergeFunctionsPass"));
}
