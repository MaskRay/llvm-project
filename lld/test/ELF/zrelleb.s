...
# CHECK:       Relocation section '.relleb.dyn' at offset {{.*}} contains 3 entries:
...
# RUN: ld.lld -shared -z relleb -z rela a.o -o out0.so --fatal-warnings
# RUN: llvm-readelf -d -r out0.so | FileCheck %s --check-prefix=RELA

# RUN: ld.lld -v -zrelleb 2>&1 | FileCheck /dev/null --implicit-check-not=warning:
...
# RUN: ld.lld -pie -z relleb a.o b.o -o out
# RUN: llvm-readelf -rs out | FileCheck %s --check-prefix=CHECK2

# CHECK2:       Relocation section '.relleb.dyn' at offset {{.*}} contains 3 entries:
