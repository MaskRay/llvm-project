...
DynamicSection<ELFT>::computeContents() {
  ...
  if (isMain && in.relaPlt->isNeeded()) {
    ...
    addInt(DT_PLTREL, config->zCrel && config->zNow ? DT_CREL
                      : config->isRela              ? DT_RELA
                                                    : DT_REL);
  }
...
template <class uint> bool CrelSection<uint>::updateAllocSize() {
  ...
  for (size_t i = 0; i != count; ++i) {
    ...
    crels[i].r_addend = config->isRela ? rel.computeAddend() : 0;
  ...
  const size_t oldSize = relocData.size();
  relocData.clear();
  raw_svector_ostream os(relocData);
  uint offset = 0, addend = 0;
  uint32_t symidx = 0, type = 0;
  encodeVU128(count * 4 + shift, os);
  for (const auto &rel : crels) {
    ...
    offset = rel.r_offset;
    uint64_t b = deltaOffset * 8 + (symidx != rel.r_symidx) +
                 (type != rel.r_type ? 2 : 0) +
                 (addend != uint(rel.r_addend) ? 4 : 0);
    encodeVU128(b, os);
    if (b & 1) {
      encodeVS128(static_cast<int32_t>(rel.r_symidx - symidx), os);
      symidx = rel.r_symidx;
    }
    if (b & 2) {
      encodeVS128(static_cast<int32_t>(rel.r_type - type), os);
      type = rel.r_type;
    }
    if (b & 4) {
      encodeVS128(std::make_signed_t<uint>(rel.r_addend - addend), os);
      addend = rel.r_addend;
    }
  }
