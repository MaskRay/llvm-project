...
inline uint64_t readVU128(const uint8_t *&p) {
  uint8_t b = *p++;
  if (b < 0xf0)
    return b;
  b &= 0x0f;
...
inline int64_t readVS128(const uint8_t *&p) {
  uint64_t v = readVU128(p);
  return (v >> 1) ^ -(v & 1);
}

template <class RelTy> struct Relocs : ArrayRef<RelTy> {
  using ArrayRef<RelTy>::ArrayRef;
  Relocs(ArrayRef<RelTy> a) : ArrayRef<RelTy>(a) {}
};

template <typename uint> struct RelocsCrel {
  struct const_iterator {
    uint32_t count;
    uint8_t shift;
    const uint8_t *p;
    llvm::object::Elf_Crel_Impl<sizeof(uint) == 8> crel{};
    const_iterator(size_t hdr, const uint8_t *p)
        : count(hdr / 4), shift(hdr % 4), p(p) {
      if (count)
        step();
    }
    void step() {
      const uint64_t b = readVU128(p);
      crel.r_offset += b >> 3 << shift;
      if (b & 1)
        crel.r_symidx += readVS128(p);
      if (b & 2)
        crel.r_type += readVS128(p);
      if (b & 4)
        crel.r_addend += static_cast<uint>(readVS128(p));
    }
    llvm::object::Elf_Crel_Impl<sizeof(uint) == 8> operator*() const {
      return crel;
    };
    const llvm::object::Elf_Crel_Impl<sizeof(uint) == 8> *operator->() const {
      return &crel;
    }
  ...
  RelocsCrel(const uint8_t *p) : hdr(readVU128(p)) { this->p = p; }
  size_t size() const { return hdr / 4; }
  const_iterator begin() const { return {hdr, p}; }
  const_iterator end() const { return {0, nullptr}; }
};

template <>
struct Relocs<llvm::object::Elf_Crel_Impl<false>> : RelocsCrel<uint32_t> {
  using RelocsCrel<uint32_t>::RelocsCrel;
};
template <>
struct Relocs<llvm::object::Elf_Crel_Impl<true>> : RelocsCrel<uint64_t> {
  using RelocsCrel<uint64_t>::RelocsCrel;
};
