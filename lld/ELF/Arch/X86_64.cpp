...
void X86_64::scanSectionImpl(InputSectionBase &sec, Relocs<RelTy> rels) {
  ...
  for (auto it = rels.begin(); it != rels.end(); ++it) {
    ...
    RelExpr expr = rs.scanOne<X86_64, ELFT>(*this, symIdx, sym, offset, type);
    if (expr == R_NONE)
      continue;
    ...
    case R_DTPREL:
      ...
      continue;
    case R_GOT_PC:
      if (!sym.isTls()) // Fall back to RelocScan::process
        break;
    ...
    case R_TLSLD_PC:
      ...
      continue;
    case R_TLSDESC_CALL:
      // Use R_TPREL for both initial-exec and local-exec optimizations as the
