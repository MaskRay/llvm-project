...
void X86::scanSectionImpl(InputSectionBase &sec, Relocs<RelTy> rels) {
  ...
  for (auto it = rels.begin(); it != rels.end(); ++it) {
    ...
    RelExpr expr = rs.scanOne<X86, ELFT>(*this, symIdx, sym, offset, type);
    if (expr == R_NONE)
      continue;
    ...
    case R_GOTPLT:
      ctx.in.gotPlt->hasGotPltOffRel.store(true, std::memory_order_relaxed);
      // Handle TLS IE relocation R_386_TLS_GOTIE (@gotntpoff).
      if (sym.isTls()) {
    ...
    case R_TPREL_NEG:
      ...
      break;
    case R_DTPREL:
      sec.addReloc({isShared ? expr : R_TPREL, type, offset, addend, &sym});
      continue;
    case R_GOT:
      // Handle TLS IE relocation R_386_TLS_IE (@indntpoff).
      if (!sym.isTls())
        break;
      ctx.hasTlsIe.store(true, std::memory_order_relaxed);
      if (!isShared && !sym.isPreemptible) {
        ...
        if (ctx.arg.isPic)
          sec.getPartition(ctx).relaDyn->addRelativeReloc(
              ctx.target->relativeRel, sec, offset, sym, addend, type, expr);
        else
          sec.addReloc({expr, type, offset, addend, &sym});
      }
      continue;
    case R_TLSGD_GOTPLT:
      ...
      if (isShared) {
        ...
        if (sym.isPreemptible) {
          ...
          sec.addReloc({R_TPREL_NEG, type, offset, addend, &sym});
        }
        ++it; // Skip the __tls_get_addr call
      }
      continue;
    case R_TLSLD_GOTPLT:
      ctx.in.gotPlt->hasGotPltOffRel.store(true, std::memory_order_relaxed);
    ...
    case R_TLSDESC_GOTPLT:
      ...
      if (isShared) {
        ...
        // TLSDESC uses positive TP offset (unlike TLS_GD).
        sec.addReloc({R_TPREL, type, offset, addend, &sym});
      }
      continue;
    case R_TLSDESC_CALL:
