...
public:
  ...
  // Used by non-alloc SHT_CREL to hold the total count and content byte
  // stream.
  uint64_t crelCountShift = 0;
  ...
  template <class uint> void finalizeNonAllocCrelImpl();
  void finalizeNonAllocCrel();
  void finalize();
  template <class ELFT>
