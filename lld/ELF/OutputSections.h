...
public:
  ...
  // Used by non-alloc SHT_RELLEB to hold the total count and content byte
  // stream.
  uint64_t rellebCount = 0;
  SmallVector<char, 0> rellebBody;
  ...
  template <class uint> void finalizeNonAllocRellebImpl();
  void finalizeNonAllocRelleb();
  void finalize();
  template <class ELFT>
