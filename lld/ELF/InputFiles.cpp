...
static GnuPropertyInfo readGnuProperty(Ctx &, const InputSection &,
                                       ObjFile<ELFT> &);
...
template <class ELFT>
static GnuPropertyInfo readGnuProperty(Ctx &ctx, const InputSection &sec,
                                       ObjFile<ELFT> &f) {
  ...
  while (!data.empty()) {
    ...
    if (data.size() < sizeof(Elf_Nhdr) ||
      ...
      return (err(data.data()) << "data is too short", GnuPropertyInfo{});
