...
// firstObj/firstShared/firstBitcode name an already-accepted file of each kind
// for the fallback diagnostic; readSymbols tracks them while scanning.
...
void ObjFile<ELFT>::scanEarlySections(EarlySectionInfo &out) {
  ...
  for (size_t i = 0, size = objSections.size(); i != size; ++i) {
    ...
    if (sec.sh_type == SHT_GROUP) {
      ...
      if (entries->empty() || (*entries)[0] != Elf_Word(GRP_COMDAT) ||
          sec.sh_info >= eSyms.size() ||
          eSyms[sec.sh_info].st_name >= stringTable.size())
        continue;
      comdatSecs.push_back({(uint32_t)i, sec.sh_info});
      continue;
    }
...
void ObjFile<ELFT>::initializeSections(bool ignoreComdats,
  ...
  // registered by the parse pipeline (Pipeline::commitFiles). keptGroups
  // memoizes the verdict for the main loop below (recomputing it there would
  // re-decode and re-hash every signature). A group scanned by
  // scanEarlySections (the comdatSecs cursor) is a valid GRP_COMDAT group:
  // reuse the cached signature hash and re-read only the entries.
  ...
  for (size_t i = 0; i != size; ++i) {
    ...
    while (comdatIdx != comdatSecs.size() && comdatSecs[comdatIdx].secIdx < i)
      ++comdatIdx;
    if (comdatIdx != comdatSecs.size() && comdatSecs[comdatIdx].secIdx == i) {
      entries = cantFail(
          this->getObj().template getSectionContentsAsArray<Elf_Word>(sec));
      keepGroup = ignoreComdats || comdatSecs[comdatIdx].prevailing;
      ++comdatIdx;
    } else {
...
// are registered under name@version instead, if at all. Used by the pipeline's
// readShared.
//
...
// 3. Activation engine: order-independent (mold/wild-style) archive extraction.
//    A non-weak undefined reference to a name with no eager definition extracts
//    the first lazy definition of that name, to a fixpoint; the whole link is
//    one implicit group. This abandons GNU positional semantics (backward
//    references resolve; --start-group is a no-op).
...
//
// A serial epilogue (Pipeline::epilogue) runs the order-sensitive side effects
// in command-line order: register shared and bitcode files and their comdat
// groups, deduplicate sonames, process dependent libraries, parse binary
// blobs, and replay -y traced resolutions. A final parallel phase
// (Pipeline::initSections) initializes the sections and local symbols of the
// eager object files, leaving the batch fully parsed.
//
//===----------------------------------------------------------------------===//
...
// A symbol's serial insertion point, used to order the installed symbol vector
// so the output .symtab matches the serial linker.
struct OrderItem {
  uint32_t ord;
  uint64_t sub;
...
template <class ELFT> struct Pipeline {
  ...
  size_t firstObjFile;                // this batch's start in ctx.objectFiles
  bool hasTraced = false;
  // A late batch (dependent libraries, LTO outputs, reactivate) extends the
  // installed symbol table in place rather than rebuilding the whole symbol
  // vector.
  bool incremental;
...
template <class ELFT> void Pipeline<ELFT>::readSymbols() {
  ...
  // entirely, as in the serial linker. For each compatible file mark it
  // eligible and, for bitcode, save its symbol names through the string savers
  // serially (they are not thread-safe; this also gives LTO NUL-terminated
  // persistent names). The first non-lazy file of each kind feeds the
  // compatibility check, so record it after.
...
template <class ELFT> void Pipeline<ELFT>::readObj(uint32_t i) {
  ...
  // Derive the comdat signatures. A global signature symbol reuses its
  // record's name length and hash (the hash is of the stem, so a rare
  // '@'-containing name rehashes); a local one is read from the string table.
  auto sigOf = [&](uint32_t symIdx) {
    if (uint32_t k = symIdx - firstGlobal; k < fd[i].symOrder.size()) {
      const SymRecord &rec = fd[i].records[fd[i].symOrder[k]];
...
template <class ELFT> void Pipeline<ELFT>::buildNameDB() {
  ...
  // against them. The shard maps' keys are the original registration stems: a
  // symbol's current name may differ after version parsing, but a record
  // still keys by the stem. Shard routing and bucket routing share the name
  // hash, so each bucket seeds from exactly its own shard.
  ...
  parallelFor(0, numShards, [&](size_t b) {
    ...
    // Shard loads deviate several percent from the mean (heavily referenced
    // names concentrate), so size refs exactly rather than paying a mid-build
    // reallocation in the shards above it.
    size_t numRefs = 0;
    for (const FileData &d : fd)
      numRefs += d.bucketStart[b + 1] - d.bucketStart[b];
    bu.names.reserve(total / numShards / 4 + shard.size());
    bu.refs.reserve(numRefs);
    bu.map.reserve(total / numShards / 4 + shard.size());
...
template <class ELFT> void Pipeline<ELFT>::activate() {
  // Order-independent (mold/wild-style) archive extraction: a lazy member is
  // pulled in if a non-weak undefined reference anywhere names a symbol whose
  // first definition is that member, computed to a fixpoint regardless of
  // command-line order. The whole link behaves like one implicit group, so
  // backward references resolve and --start-group is a no-op. Determinism comes
  // from file index, not scan order: an extracted member ranks at its
  // command-line position and resolves like any eager object. This abandons GNU
  // positional semantics -- --why-extract and which member wins among
  // duplicate-defining archives can differ from the serial linker.
  ranks.assign(files.size(), UINT32_MAX);
  ...
  // Extraction decisions are a pure function of the pre-activation lazy
  // flags; snapshot them so that summaries stay order-independent while
  // extraction clears files[i]->lazy.
  SmallVector<uint8_t, 0> wasLazy(files.size());
  for (auto [i, f] : llvm::enumerate(files))
    wasLazy[i] = f->lazy;

  // Summarize a name by walking its definition chain (built in (file, symbol
  // index) order): the first definition per tier (tier 0 = strong/regular,
  // tier 1 = weak/tentative), whether it is lazy, whether the tier has an
  // eager regular (non-shared) definition, and the first lazy non-tentative
  // definition (the --fortran-common override target).
  struct DefInfo {
  ...
  auto summarize = [&](uint32_t b, uint32_t nameId) {
    ...
    for (uint32_t r = bu.names[nameId].firstDef; r != UINT32_MAX;
      ...
      if (!lazy && !(rec.flags & FShared))
        di.eagerReg[t] = true;
      if (lazy && di.lazyStrongDef == UINT32_MAX && !(rec.flags & FCommon))
  ...
  // definition wins, and the member is extracted only if it is lazy. This
  // matches mold/ld.bfd: a regular-object definition beats a shared one when
  // seen first, an eager regular definition is always preferred to a lazy one,
  // and a weak archive member is pulled in only when nothing stronger defines
  // the name.
  ...
  // whose own non-weak undefined references join the worklist. trig records the
  // first referrer for --why-extract. extractTarget is fixed, so each name maps
  // to a single member and the live set is order-independent.
  SmallVector<uint32_t, 0> trig(numNames);
  SmallVector<uint8_t, 0> seen(numNames);
  struct WorkItem {
    uint32_t bucket, nameId;
  };
  ...
  // The sentinel trigger file index files.size() denotes ctx.internalFile.
  ...
  // --fortran-common: a name whose COMMON definition is active (in an eager
  // file or an extracted member) pulls its lazy strong definition to override
  // it. 0 = unchecked, 1 = override wanted, 2 = checked, no lazy strong
  // definition.
  SmallVector<uint8_t, 0> wantOverride;
  if (fortranCommon)
    wantOverride.assign(numNames, 0);
  auto markOverride = [&](const SymRecord &rec) {
    uint32_t id = globalId(rec);
    if (wantOverride[id])
      return;
    uint32_t b = rec.hash % numShards;
    if (summarize(b, rec.nameId).lazyStrongDef == UINT32_MAX) {
      wantOverride[id] = 2;
      return;
    }
    wantOverride[id] = 1;
    work.push_back({b, rec.nameId});
  };

  if (!triggers.empty()) {
    // A reactivate batch pulls in only the members defining the explicit
    // triggers (and, transitively, their references).
    ...
    // Seeded -u references and every non-weak undefined reference of an eager
    // (non-lazy) file.
    ArrayRef<Symbol *> symVec = ctx.symtab->getSymbols();
    for (const auto &shard : ctx.symtab->getShards())
      for (const auto &kv : shard) {
        Symbol *s = symVec[kv.second];
    ...
    for (auto [i, f] : llvm::enumerate(files)) {
      ...
      for (uint32_t ri : fd[i].symOrder) {
        ...
        else if (fortranCommon && (rec.flags & FCommon))
          markOverride(rec);
      }
    }
  }
  ...
  while (!work.empty()) {
    ...
    // A --fortran-common override pulls a lazy strong definition over an active
    // COMMON; otherwise a referenced name pulls its first-seen lazy member.
    DefInfo di = summarize(b, nameId);
    uint32_t m;
    if (fortranCommon && wantOverride[id] == 1)
      m = di.lazyStrongDef;
    else
      m = extractTarget(di);
    ...
    for (uint32_t ri : fd[m].symOrder) {
      const SymRecord &rec = fd[m].records[ri];
      if (!(rec.flags & (FDef | FWeak)))
  ...
  // Assign a visit rank to each compatible file in command-line (file index)
  // order. The downstream phases iterate the compatible files directly: an
  // extracted/eager file (!lazy) is a full parse, a still-lazy file contributes
  // only LazySymbol definitions.
  uint32_t seq = 1;
...
template <class ELFT> void Pipeline<ELFT>::commitFiles() {
  // Register object files in command-line (file index) order (this determines
  // output section order), pre-populate the comdat group shards, and print -t
  // traces for the initial batch. The eager (extracted/non-lazy) compatible
  // files are the full parses.
  ...
  for (auto [i, f] : llvm::enumerate(files)) {
    ...
    if (ctx.arg.trace && !ignoreComdats)
      Msg(ctx) << f;
    if (fd[i].eligible && f->kind() == InputFile::ObjKind)
...
// Resolve one symbol record against its already-created symbol, dispatching on
// the source file kind. This is the sole symbol-resolution dispatch, shared by
// the parallel resolveName and the serial -y traced replay in the epilogue, so
...
void Pipeline<ELFT>::applyRecord(Symbol *sym, InputFile *file,
  ...
  if (rec.flags & FShared) {
    ...
    // An unextracted lazy object or bitcode contributes only its definitions,
    // as LazySymbols (ObjFile/BitcodeFile::parseLazy). Extraction is decided by
    // activate; an earlier shared or bitcode definition suppresses the
    // LazySymbol.
...
// Replay resolution events for -y traced names of one file in serial
// (symbol-table) order, deferred by resolveName so the trace output is
// deterministic. The order matches resolveName's within-file order:
// `phaseSplit` visits definitions before undefined references (objects and
// bitcode); `defsOnly` restricts to definitions (an unextracted lazy member's
// undefined references are never inserted).
template <class ELFT>
...
void Pipeline<ELFT>::resolveName(Bucket &bu, uint32_t nameId) {
  ...
  // Merge the definition and undefined-reference chains, both built in
  // (visit rank, symbol index) order, into a single event stream ordered by
  // (rank, definitions first, symbol index), so resolution matches the serial
  ...
  // the file that first inserts the symbol. Object symbols are inserted in
  // symbol-table order; bitcode files insert definitions before undefined
  // references (BitcodeFile::parse); a shared file inserts each symbol's
  // unversioned name before its versioned one (the versioned record carries
  // FHasAt from its '@'). -y defers the real resolution of traced names to
  // the serial replay in the epilogue.
  ...
  auto key = [&](uint32_t r, uint64_t undefPhase) {
    const RefNode &node = bu.refs[r];
    return (uint64_t(ranks[node.fileIdx]) << 33) | (undefPhase << 32) |
...
template <class ELFT> void Pipeline<ELFT>::resolveSymbols() {
  ...
  // the final storage so the output .symtab matches the serial linker. Names
  // that no serial insertion event would have created (e.g. names only
  // referenced by undefined symbols of unextracted lazy members) are dropped
  // before the sort: they must not appear in symVector or the installed maps.
  uint32_t liveStart[numShards + 1];
  {
    uint32_t counts[numShards];
    parallelFor(0, numShards, [&](size_t b) {
      uint32_t n = 0;
      for (const NameInfo &ni : buckets[b].names)
        n += ni.anchorRank != UINT32_MAX;
      counts[b] = n;
    });
    uint32_t sum = 0;
    for (uint32_t b = 0; b != numShards; ++b) {
  ...
  size_t live = liveStart[numShards];
  SmallVector<OrderItem, 0> items;
...
template <class ELFT> void Pipeline<ELFT>::recordExtractions() {
  // Record --why-extract attribution: the trigger reference and the member it
  // extracted.
...
template <class ELFT> void Pipeline<ELFT>::epilogue() {
  // Parse non-object files in command-line (file index) order, process
  // dependent libraries and ARM attributes of object files, and replay
  // resolution events for -y traced symbols. A still-lazy compatible file
  // contributes only LazySymbol insertions; the rest are full parses.
  for (auto [i, f] : llvm::enumerate(files)) {
    if (!fd[i].compatible)
      continue;
    FileData &d = fd[i];
    if (f->lazy) {
      ...
      // Lazy object: replay LazySymbol insertions for traced names.
    ...
    if (auto *sf = dyn_cast<SharedFile>(f); sf && d.eligible) {
      ...
      sf->allocateSymbols();
      sf->template parseGnuAndFeatures<ELFT>(sf->template getObj<ELFT>());
      if (hasTraced)
...
template <class ELFT> void Pipeline<ELFT>::initSections() {
  // Initialize the sections and local symbols of this batch's object files
  // (registered by commitFiles). This runs after the epilogue, so that comdat
  // group ownership and ctx.in.attributes are final, and after the phase-local
  // data is freed, so that section and local symbol allocations reuse it.
  // Schedule the most expensive files (sections plus local symbols) first.
  ArrayRef<ELFFileBase *> objs = ArrayRef(ctx.objectFiles).slice(firstObjFile);
  auto cost = [&](uint32_t i) -> uint64_t {
    auto *f = cast<ObjFile<ELFT>>(objs[i]);
    return f->template getELFShdrs<ELFT>().size() + f->getFirstGlobal();
  };
...
doParseFiles(Ctx &ctx,
  ...
  for (size_t done = 0; done < files.size();) {
    ...
    Pipeline<ELFT> p(ctx, std::move(batch), /*incremental=*/done != 0);
    p.run();
    done = end;
  }
...
static void doParseLtoObjectFiles(Ctx &ctx, ArrayRef<InputFile *> files) {
  Pipeline<ELFT> p(ctx, SmallVector<InputFile *, 0>(files.begin(), files.end()),
                   /*incremental=*/true, /*triggers=*/{},
                   /*ignoreComdats=*/true);
  p.run();
  ...
  for (InputFile *f : files)
    for (Symbol *sym : cast<ObjFile<ELFT>>(f)->getGlobalSymbols())
...
template <class ELFT>
static void doReactivate(Ctx &ctx, SmallVector<InputFile *, 0> lazyFiles,
                         ArrayRef<Symbol *> triggers) {
  Pipeline<ELFT> p(ctx, std::move(lazyFiles), /*incremental=*/true, triggers);
  p.run();
}
...
void elf::reactivate(Ctx &ctx, ArrayRef<Symbol *> triggers) {
  ...
  if (lazyFiles.empty())
    return;
  invokeELFT(doReactivate, ctx, std::move(lazyFiles), triggers);
}
