...
void OutputSection::writeTo(uint8_t *buf, parallel::TaskGroup &tg) {
  ...
  if (type == SHT_CREL && !(flags & SHF_ALLOC)) {
    buf += encodeVU128(crelHeader, buf);
...
encodeOneCrel(raw_svector_ostream &os, uint &outOffset, uint32_t &outSymidx,
  ...
  uint64_t b = deltaOffset * 8 + (outSymidx != symidx) +
               (outType != type ? 2 : 0) + (outAddend != addend ? 4 : 0);
  encodeVU128(b, os);
  if (b & 1) {
    encodeVS128(static_cast<int32_t>(symidx - outSymidx), os);
    outSymidx = symidx;
  }
  if (b & 2) {
    encodeVS128(static_cast<int32_t>(type - outType), os);
    outType = type;
  }
  if (b & 4) {
    encodeVS128(std::make_signed_t<uint>(addend - outAddend), os);
    outAddend = addend;
  }
}
...
static size_t relToCrel(raw_svector_ostream &os, typename ELFT::uint &outOffset,
  ...
  if (relSec->type == SHT_REL) {
    ...
    // FIXME zero addend may not be desired.
    //
...
template <class uint> void OutputSection::finalizeNonAllocCrelImpl() {
  ...
  for (InputSection *relSec : isd->sections) {
    ...
    const auto hdr = readVU128(p);
    const size_t count = hdr / 4, shift = hdr % 4;
    totalCount += count;
    ...
    for (size_t i = 0; i != count; ++i) {
      ...
      encodeOneCrel(os, outOffset, outSymidx, outType, outAddend,
                    (uint)sec->getVA(offset << shift), file.getSymbol(symidx),
                    type, addend);
    }
  }

  crelHeader = totalCount * 4;
  size = getVU128Size(crelHeader) + crelBody.size();
}
