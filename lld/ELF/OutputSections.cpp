  ...
  if (LLVM_UNLIKELY(type != isec->type)) {
    if (!hasInputSections && !typeIsSet) {
      type = isec->type;
               ...
               (type == SHT_RELLEB) != (isec->type == SHT_RELLEB)) {
      // Combine mixed SHT_REL[A] and SHT_RELLEB to SHT_RELLEB.
      type = SHT_RELLEB;
      if (type == SHT_REL) {
        if (name.consume_front(".rel"))
          name = saver().save(".relleb" + name);
...
void OutputSection::writeTo(uint8_t *buf, parallel::TaskGroup &tg) {
  ...
  if (type == SHT_NOBITS)
    return;
  if (type == SHT_RELLEB && !(flags & SHF_ALLOC)) {
    buf += encodeULEB128(rellebCount, buf);
    memcpy(buf, rellebBody.data(), rellebBody.size());
    return;
  }
...
encodeOneRelleb(raw_svector_ostream &os, uint &outOffset, uint32_t &outSymidx,
                uint32_t &outType, uint &outAddend, uint offset,
                const Symbol &sym, uint32_t type, uint addend) {
  const auto deltaOffset = static_cast<uint64_t>(offset - outOffset) * 2;
  outOffset = offset;
  ...
  if (outSymidx == symidx && outType == type) {
    encodeULEB128(deltaOffset + 1, os);
    encodeSLEB128(std::make_signed_t<uint>(addend - outAddend), os);
    outAddend = addend;
    return;
  }
  encodeULEB128(deltaOffset, os);
  outSymidx = symidx;
  if (outType == type && outAddend == addend) {
...
template <class ELFT>
static size_t relToRelleb(raw_svector_ostream &os,
                          typename ELFT::uint &outOffset, uint32_t &outSymidx,
                          uint32_t &outType, typename ELFT::uint &outAddend,
  ...
  if (relSec->type == SHT_REL) {
    ...
    for (auto rel : rels) {
      encodeOneRelleb<typename ELFT::uint>(os, outOffset, outSymidx, outType,
    ...
    for (auto rel : rels) {
      encodeOneRelleb<typename ELFT::uint>(
...
template <class uint> void OutputSection::finalizeNonAllocRellebImpl() {
  raw_svector_ostream os(rellebBody);
  ...
  for (InputSection *relSec : isd->sections) {
    ...
    if (relSec->type != SHT_RELLEB) {
      if constexpr (sizeof(uint) == 4) {
        totalCount +=
            config->isLE
                ? relToRelleb<ELF32LE>(os, outOffset, outSymidx, outType,
                                       outAddend, relSec, sec)
                : relToRelleb<ELF32BE>(os, outOffset, outSymidx, outType,
                                       outAddend, relSec, sec);
      } else {
        totalCount +=
            config->isLE
                ? relToRelleb<ELF64LE>(os, outOffset, outSymidx, outType,
                                       outAddend, relSec, sec)
                : relToRelleb<ELF64BE>(os, outOffset, outSymidx, outType,
                                       outAddend, relSec, sec);
      }
      continue;
    }
    ...
    for (size_t i = 0; i != count; ++i) {
      ...
      encodeOneRelleb(os, outOffset, outSymidx, outType, outAddend,
                      (uint)sec->getVA(offset), file.getSymbol(symidx), type,
                      addend);
    }
  }

  rellebCount = totalCount;
  size = getULEB128Size(totalCount) + rellebBody.size();
}

void OutputSection::finalizeNonAllocRelleb() {
  if (config->is64)
    finalizeNonAllocRellebImpl<uint64_t>();
  else
    finalizeNonAllocRellebImpl<uint32_t>();
}
