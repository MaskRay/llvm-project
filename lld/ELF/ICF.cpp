...
private:
  ...
  SmallVector<uint32_t, 0> eqClass;
  SmallVector<uint32_t, 0> nextEqClass;
...
template <class ELFT> void ICF<ELFT>::run() {
  ...
  uint32_t uniqueId = 0;
  ctx.in.ehFrame->iterateFDEWithLSDA<ELFT>([&](InputSection &s) {
  ...
  // Change Defined symbol's section field to the canonical one. Only folded
  // sections are keys in icfRepl and they are all dead (markDead above), so a
  // live section is never a key; skip the map lookup for the common live case.
