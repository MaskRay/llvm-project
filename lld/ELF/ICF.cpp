...
//
// Step 1 is cheap and leaves nearly exact classes, so step 2 converges in a
// few iterations, even for programs as large as Chromium. Step 2 is what
// makes the result exact: it does not rely on the absence of hash collisions.
//
...
// A section referencing a symbol it defines contributes its own hash, so this
// must not map equal arguments to a constant.
static uint64_t mix(uint64_t h, uint64_t v) {
  h = h * 0x9e3779b97f4a7c15 + v;
  return h ^ (h >> 32);
}

// Two different symbols can only compare equal if both have a final value: not
// a linker script placeholder, which may change later, and not preemptible,
// which may resolve to another definition at runtime.
...
// MergeInputSection. splitSections pre-resolves a non-section symbol's value,
// but a section symbol's addend is arbitrary and is not validated until
// getSymVA, which maps an offset at or past the end to 0. Bound it here, as
// getSectionPiece would otherwise index pieces out of bounds; -1 keeps such a
// relocation from comparing equal to one with a real offset.
static uint64_t getMergeOffset(const Defined &d, uint64_t addend) {
  auto &ms = cast<MergeInputSection>(*d.section);
  if (!d.isSection())
    return ms.getOffset(d.value) + addend;
  if (addend >= ms.content().size())
    return -1;
  return ms.getOffset(addend);
}
...
// edges instead and contribute through hash propagation. Hashing must never
// distinguish two sections that compare equal; the converse is free, as
// segregate() is exact, so equalsConstant's getParent() need not be hashed.
template <class ELFT>
template <class RelTy>
...
template <class ELFT> void ICF<ELFT>::run() {
  ...
  assert(sections.size() <= (1u << 31) && uniqueId < (1u << 31) &&
         "the MSB marks a candidate and the low bits index sections");
  ...
  // Propagate hashes along edges. Each round covers one more relocation hop and
  // removes one segregate() pass, trading exact comparison for cheap hashing.
  // 6 is empirical: most programs then need just two passes.
