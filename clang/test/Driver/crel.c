...
// RUN: %clang -### -c --target=x86_64 -Wa,--crel,--allow-experimental-crel %s 2>&1 | FileCheck %s
// RUN: %clang -### -c --target=x86_64 -Wa,--crel,--no-crel,--allow-experimental-crel %s 2>&1 | FileCheck %s --check-prefix=NO
