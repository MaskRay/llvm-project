...
static void addX86AlignBranchArgs(const Driver &D, const ArgList &Args,
  ArgStringList &CmdArgs) {
  unsigned AlignBranchBoundary = 0, AlignBranchPrefixSize = 0;
  std::string AlignBranch;
  if (Args.hasArg(options::OPT_mbranches_within_32B_boundaries)) {
    AlignBranchBoundary = 32;
    AlignBranch = "fused,jcc,jmp";
    AlignBranchPrefixSize = 5;
  }
  ...
  if (const Arg *A =
    ...
    StringRef Value = A->getValue();
    if (Value.getAsInteger(10, AlignBranchPrefixSize) ||
        AlignBranchPrefixSize > 5)
      D.Diag(diag::err_drv_invalid_argument_to_option)
          << Value << A->getOption().getName();
  }
  if (AlignBranchBoundary) {
    CmdArgs.push_back("-mllvm");
    CmdArgs.push_back(Args.MakeArgString("-x86-align-branch-boundary=" +
                                         Twine(AlignBranchBoundary)));
    CmdArgs.push_back("-mllvm");
    std::replace(AlignBranch.begin(), AlignBranch.end(), ',', '+');
